...
public class VesselCharterContractDefaultClassImporter extends DefaultClassImporter {
	...
	private void exportWithVesselNameAndCharterNumber(final Map<String, String> result, EObject parent) {
		...
		if (parentParent instanceof VesselCharter vesselCharter) {
			Vessel vessel = vesselCharter.getVessel();
			if (vessel != null) {
				result.put(VESSEL_NAME_FIELD, vessel.getName());
			}
			result.put(CHARTER_NUMBER_FIELD, Integer.toString(vesselCharter.getCharterNumber()));
		}
	}
	...
	public ImportResults importObject(final EObject parent, final EClass eClass, final Map<String, String> row, final IMMXImportContext context) {
		...
		if (object != null && vesselName != null) {
			context.doLater(new VesselCharterCharterContractImporterDeferment(pCharterNo, vesselName, object));
		}

		return result;
	}
	
	private class VesselCharterCharterContractImporterDeferment implements IDeferment {
		private final int pCharterNumber;
		private final String vesselName;
		private final EObject object;
		
		public VesselCharterCharterContractImporterDeferment(final int pCharterNumber, final String vesselName, final EObject object) {
			this.object = object;
		...
		public void run(final IImportContext importContext) {
			...
			if (rootObject instanceof LNGScenarioModel) {
				...
				for (final VesselCharter vesselCharter : cargoModel.getVesselCharters()) {
					if (vesselCharter.getVessel() == targetVessel) {
						if (vesselCharter.getCharterNumber() == pCharterNumber) {
							if (object instanceof GenericCharterContract) {
								vesselCharter.setContainedCharterContract((GenericCharterContract) object);
								break;
							}
							
							GenericCharterContract contract = vesselCharter.getContainedCharterContract();
							if (contract == null) {
								contract = CommercialFactory.eINSTANCE.createGenericCharterContract();
								contract.setName("-");
								vesselCharter.setContainedCharterContract(contract);
							} else {
