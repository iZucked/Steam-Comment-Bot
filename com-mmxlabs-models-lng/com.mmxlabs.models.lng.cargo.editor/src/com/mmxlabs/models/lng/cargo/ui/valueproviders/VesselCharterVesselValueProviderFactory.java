...
public class VesselCharterVesselValueProviderFactory implements IReferenceValueProviderFactory {
	@Override
			...
			return new SimpleReferenceValueProvider(fleetModel, FleetPackage.Literals.FLEET_MODEL__VESSELS) {
				...
				private List<Pair<String, EObject>> getRestrictedVesselsList(final CargoModel cargoModel, final FleetModel fleetModel, final EObject target) {
					...
					if (target instanceof VesselCharter) {
						currentValue = ((VesselCharter) target).getVessel();
					}
					...
					if (currentValue == null) {
						...
						for (final VesselCharter availability : cargoModel.getVesselCharters()) {
