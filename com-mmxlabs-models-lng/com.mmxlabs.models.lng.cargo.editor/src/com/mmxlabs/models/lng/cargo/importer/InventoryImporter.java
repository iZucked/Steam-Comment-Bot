...
public class InventoryImporter extends DefaultClassImporter {
	...
	public ImportResults importObject(final EObject parent, final EClass eClass, final Map<String, String> row, final IMMXImportContext context) {
		...
		if (KEY_FLOW_TYPE_FEED.equalsIgnoreCase(rowType)) {
			rowClass = getTrueOutputClass(eClass, "InventoryFeedRow");
		} else if (KEY_FLOW_TYPE_OFFTAKE.equalsIgnoreCase(rowType)) {
			rowClass = getTrueOutputClass(eClass, "InventoryOfftakeRow");
