 ...
 * 
 */
public class CreatePaperStripDialog extends FormDialog {

	public static enum StripType {
		TYPE_PAPER_BUY("Buy Paper"), TYPE_PAPER_SELL("Sell Paper");
	...
	public CreatePaperStripDialog(@NonNull final IShellProvider parentShell, @NonNull final IScenarioEditingLocation originalScenarioEditingLocation, @NonNull final StripType stripType,
	...
	public CreatePaperStripDialog(@NonNull final Shell parentShell, @NonNull final IScenarioEditingLocation originalScenarioEditingLocation, @NonNull final StripType stripType,
			@Nullable final EObject selectedObject) {
		super(parentShell);
		this.scenarioEditingLocation = createScenarioEditingLocation(originalScenarioEditingLocation);
		this.stripType = stripType;
	...
	private void createSample(@Nullable final EObject selectedObject) {
		...
		switch (stripType) {
		case TYPE_PAPER_SELL:
			sample = CargoFactory.eINSTANCE.createSellPaperDeal();
			//sample.eSet(CargoPackage.eINSTANCE.getLoadSlot_DESPurchase(), Boolean.TRUE);
			break;
		case TYPE_PAPER_BUY:
			sample = CargoFactory.eINSTANCE.createBuyPaperDeal();
			break;
		default:
			break;
		}
		...
		if (selectedObject != null) {
			for (final EStructuralFeature f : sample.eClass().getEAllStructuralFeatures()) {
				...
				// skip paper ref
				if (f == CargoPackage.eINSTANCE.getPaperDeal()) {
					continue;
				}

				//FIXME : ask Simon about this one
	...
	protected void createFormContent(final IManagedForm mform) {
		...
		switch (stripType) {
		case TYPE_PAPER_SELL:
			referenceClass = CargoPackage.eINSTANCE.getSellPaperDeal();
			title += "Sell Papers";
			break;
		case TYPE_PAPER_BUY:
			referenceClass = CargoPackage.eINSTANCE.getBuyPaperDeal();
			title += "Buy Papers";
			break;
		default:
			break;
	...
	private void createFormArea(final IManagedForm mform) {
		...
		final Composite patternComposite = toolkit.createComposite(body);
		{
			...
			{
				label2 = toolkit.createLabel(patternComposite, "papers(s) between");
				final GridData gd = new GridData();
				gd.widthHint = 80;
			...
			repeatType.addSelectionChangedListener(new ISelectionChangedListener() {
				...
				public void selectionChanged(final SelectionChangedEvent event) {
					...
					if (rt == RepeatType.Periodic) {
						label1.setText("Create papers every");
						((GridData) label1.getLayoutData()).widthHint = 96;
						label2.setVisible(false);
						label3.setVisible(true);
						intervalType.getControl().setVisible(true);

						//FIXME : why do have this strange castings?
						if (label2.getLayoutData() != null) {
							((GridData) label2.getLayoutData()).exclude = true;
						}
						if (label3.getLayoutData() != null) {
							((GridData) label3.getLayoutData()).exclude = false;
						}
						if (intervalType.getControl() != null) {
							if (intervalType.getControl().getLayoutData() != null) {
...
//		}
	...
	public Command createStrip(final CargoModel cargoModel, final EditingDomain domain) {
		...
		for (final EObject eObject : generatedObjects) {
			if (CargoPackage.eINSTANCE.getSellPaperDeal().isInstance(eObject)) {
				cmd.append(AddCommand.create(domain, cargoModel, CargoPackage.eINSTANCE.getCargoModel_PaperDeals(), eObject));
			} else if (CargoPackage.eINSTANCE.getBuyPaperDeal().isInstance(eObject)) {
				cmd.append(AddCommand.create(domain, cargoModel, CargoPackage.eINSTANCE.getCargoModel_PaperDeals(), eObject));
			} else {
