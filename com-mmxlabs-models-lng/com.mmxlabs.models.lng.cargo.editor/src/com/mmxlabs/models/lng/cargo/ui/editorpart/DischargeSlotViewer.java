...
public class DischargeSlotViewer extends ScenarioTableViewerPane {
	...
	public DischargeSlotViewer(final IWorkbenchPage page, final IWorkbenchPart part, final IScenarioEditingLocation location, final IActionBars actionBars) {
	...
	public void init(final List<EReference> path, final AdapterFactory adapterFactory, final CommandStack commandStack) {
		...
		((GridTableViewer) viewer).setComparer(new IElementComparer() {
			...
			public boolean equals(final Object a, final Object b) {
				...
				final DischargeSlot c1 = getDischargeSlot(a);
				final DischargeSlot c2 = getDischargeSlot(b);

				return Equality.isEqual(c1, c2);
			}

			private DischargeSlot getDischargeSlot(final Object o) {

				if (o instanceof DischargeSlot) {
					return (DischargeSlot) o;
				}

				if (o instanceof SlotAllocation) {
					final SlotAllocation cargoAllocation = (SlotAllocation) o;
					return getDischargeSlot(cargoAllocation.getSlot());
				}
				...
				if (o instanceof SlotVisit) {
					...
					if (slotAllocation != null) {
						return getDischargeSlot(slotAllocation.getSlot());
					}
				}
				...
				if (viewer.getSelection() instanceof IStructuredSelection) {
					...
					if (structuredSelection.isEmpty() == false) {
						...
						while (itr.hasNext()) {
							...
							if (o instanceof DischargeSlot) {
								final DischargeSlot slot = (DischargeSlot) o;
	...
	// mergeAll.append(super.mergeImports(container, CargoPackage.eINSTANCE.getCargoModel_DischargeSlots(), loads));
	...
	protected ScenarioTableViewer constructViewer(final Composite parent) {
		...
		scenarioTableViewer.setColourProvider(new IColorProvider() {
			...
			public Color getBackground(final Object element) {
				...
				if (element instanceof DischargeSlot) {
					if (((DischargeSlot) element).isFOBSale()) {
						return fobCargo;
					}
				}
				return null;
