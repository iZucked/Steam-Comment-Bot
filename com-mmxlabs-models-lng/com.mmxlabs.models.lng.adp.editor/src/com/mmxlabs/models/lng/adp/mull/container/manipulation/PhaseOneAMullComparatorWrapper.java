...
@NonNullByDefault
public class PhaseOneAMullComparatorWrapper implements IMullComparatorWrapper {

	private final PhaseOneAComparator comparator;

	public PhaseOneAMullComparatorWrapper(final Map<Vessel, LocalDateTime> vesselToMostRecentUseDateTime, final Map<Vessel, LocalDateTime> vesselToNextForwardUseTime, final int defaultAllocationDrop,
			final Set<Vessel> firstPartyVessels, final int fullCargoLotValue, final int loadDuration) {
		this.comparator = new PhaseOneAComparator(vesselToMostRecentUseDateTime, vesselToNextForwardUseTime, defaultAllocationDrop, firstPartyVessels, fullCargoLotValue, loadDuration);
	}

	private class PhaseOneAComparator implements Comparator<IMudContainer> {
		...
		public PhaseOneAComparator(final Map<Vessel, LocalDateTime> vesselToMostRecentUseDateTime, final Map<Vessel, LocalDateTime> vesselToNextForwardUseTime, final int defaultAllocationDrop,
				final Set<Vessel> firstPartyVessels, final int fullCargoLotValue, final int loadDuration) {
		...
		public int compare(final IMudContainer mc0, final IMudContainer mc1) {
			...
			final int expectedAllocationDrop0 = mc0.calculateExpectedAllocationDrop(vesselToMostRecentUseDateTime, vesselToNextForwardUseTime, defaultAllocationDrop,
					this.loadDuration, firstPartyVessels, currentDateTime);
