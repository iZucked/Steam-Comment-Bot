...
public class MultipleInventoryProfileDetailComposite extends Composite implements IDisplayComposite {
	...
	private MultipleInventoryProfile oldValue = null;
	private InventorySubprofile oldSubprofile = null;
	...
	private final Adapter adapter = new MMXContentAdapter() {
		...
		public void reallyNotifyChanged(Notification notification) {
			...
			if (!isDisposed() && isVisible()) {
				...
				MultipleInventoryProfileDetailComposite.this.removeAdapter();
			}
		}
	};
	
	MultipleInventoryProfileDetailComposite(final Composite parent, final IDialogEditingContext dialogContext, final int style, final FormToolkit toolkit) {
		super(parent, style);
	...
	public void display(IDialogEditingContext dialogContext, MMXRootObject root, EObject value, Collection<EObject> range, EMFDataBindingContext dbc) {
		...
		removeAdapter();
		oldValue = (MultipleInventoryProfile) value;
	...
	private EObjectTableViewer getInventoryTableViewer(Composite parent, FormToolkit toolkit) {
		...
		addInventorySubprofileRow = new Action("Add") {
			@Override
			public void run() {
				final InventorySubprofile newLine = ADPFactory.eINSTANCE.createInventorySubprofile();
				commandHandler.handleCommand(AddCommand.create(commandHandler.getEditingDomain(), oldValue, ADPPackage.Literals.MULTIPLE_INVENTORY_PROFILE__INVENTORIES, newLine), oldValue, ADPPackage.Literals.MULTIPLE_INVENTORY_PROFILE__INVENTORIES);
				if (viewer != null) {
				...
				if (viewer != null) {
					...
					if (selection instanceof IStructuredSelection) {
						...
						for (final Object o : iStructuredSelection) {
							final InventorySubprofile line  = (InventorySubprofile) o;
		...
		eViewer.addTypicalColumn("Inventory", new SingleReferenceManipulator(ADPPackage.eINSTANCE.getInventorySubprofile_Inventory(), sel.getReferenceValueProviderCache(), sel.getEditingDomain()) {
			@Override
			public void runSetCommand(Object object, Object value) {
				super.runSetCommand(object, value);
				dialogContext.getDialogController().validate();
				eViewer.refresh();
				if (value != null && value instanceof Inventory) {
		...
		eViewer.init(sel.getAdapterFactory(), sel.getModelReference(), ADPPackage.Literals.MULTIPLE_INVENTORY_PROFILE__INVENTORIES);
		...
		eViewer.addSelectionChangedListener(event -> {
			...
			if (!event.getSelection().isEmpty()) {
				...
				if (selection instanceof InventorySubprofile) {
					InventorySubprofile selectedRow = (InventorySubprofile) selection;
	...
	private EObjectTableViewer getEntityTableViewer(Composite parent, FormToolkit toolkit) {
		...
		addEntityRow = new Action("Add") {
			...
			public void run() {
				...
				commandHandler.handleCommand(AddCommand.create(commandHandler.getEditingDomain(), oldSubprofile, ADPPackage.Literals.INVENTORY_SUBPROFILE__ENTITY_TABLE, newLine), oldSubprofile, ADPPackage.Literals.INVENTORY_SUBPROFILE__ENTITY_TABLE);
				if (viewer != null) {
					viewer.setSelection(new StructuredSelection(newLine));
					viewer.refresh();
				}
		...
		eViewer.addTypicalColumn("Entity", new SingleReferenceManipulator(ADPPackage.eINSTANCE.getInventoryADPEntityRow_Entity(), sel.getReferenceValueProviderCache(), sel.getEditingDomain()) {
			...
			public void runSetCommand(Object object, Object value) {
				...
				if (value != null && value instanceof BaseLegalEntity) {
					entityTableGroup.setText(((BaseLegalEntity) value).getName() + " market allocations");
				}
			}
		});
		...
		eViewer.init(sel.getAdapterFactory(), sel.getModelReference(), ADPPackage.Literals.INVENTORY_SUBPROFILE__ENTITY_TABLE);
		final GridData gridData = new GridData(SWT.FILL, SWT.FILL, true, true);
	...
	private EObjectTableViewer getContractTableViewer(Composite parent, FormToolkit toolkit) {
		...
		eViewer.addTypicalColumn("ACQ", new NumericAttributeManipulator(ADPPackage.eINSTANCE.getContractAllocationRow_Weight(), sel.getEditingDomain()) {
			@Override
			public void runSetCommand(Object object, Object value) {
				super.runSetCommand(object, value);
				dialogContext.getDialogController().validate();
...
//		});
		...
		eViewer.addTypicalColumn("ACQ", new NumericAttributeManipulator(ADPPackage.eINSTANCE.getMarketAllocationRow_Weight(), sel.getEditingDomain()) {
			@Override
			public void runSetCommand(Object object, Object value) {
				super.runSetCommand(object, value);
				dialogContext.getDialogController().validate();
