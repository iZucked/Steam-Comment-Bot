...
public class ImportEastOfSuezPortGroupPage extends WizardPage {
	...
	public ImportEastOfSuezPortGroupPage(final String pageName, final ScenarioInstance currentScenario) {
		super(pageName);
		this.currentScenario = currentScenario;
		setTitle("Select scenarios");
		setDescription("Choose scenario(s) to reset the east of Suez port group");
	}
	...
	public void createControl(Composite parent) {
		...
		if (currentScenario != null) {
			...
			scenarioSelectionGroup = new RadioSelectionGroup(container, "Scenarios", SWT.NONE, new String[] { "All", "Selected" },
					new int[] { ImportUtils.CHOICE_ALL_SCENARIOS, ImportUtils.CHOICE_SELECTED_SCENARIOS });
			selectedOnlyIndex = 1;
		}
	...
	protected List<ScenarioInstance> getAllDescendantScenarioInstances(final Container parent) {
		...
		for (final Object object : parent.getElements()) {
			...
			if (object instanceof Container container) {
				result.addAll(getAllDescendantScenarioInstances(container));
			}
		}
