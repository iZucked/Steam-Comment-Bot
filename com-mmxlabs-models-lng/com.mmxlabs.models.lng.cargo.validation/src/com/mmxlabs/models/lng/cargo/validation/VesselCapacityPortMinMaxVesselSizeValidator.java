...
/**
 * Check that the vessel / port combination on the slots assigned to a cargo wrt
 * the [minVesselCapacity, maxVesselCapacity] bounds for a Port.
 */
public class VesselCapacityPortMinMaxVesselSizeValidator extends AbstractModelMultiConstraint {
	...
	@NonNull
	private DetailConstraintStatusDecorator createInvalidVesselPortAssignmentStatus(final String violationMsg, final String violationType, final IValidationContext ctx, final Vessel vessel,
			final Slot slot, final Port port, int vesselSizeBound) {
		final String message = String.format("Vessel %s cannot dock at port %s as vessel capacity (%d) %s port %s vessel size (%d).", vessel.getName(), port.getName(),
				vessel.getVesselOrDelegateCapacity(), violationMsg, violationType, vesselSizeBound);
