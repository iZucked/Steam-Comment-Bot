			...
			if (cargo.getCargoType() != CargoType.FLEET) {
				...
				for (final Slot<?> s : cargo.getSlots()) {
					if (s instanceof LoadSlot loadSlot) {
						if (loadSlot.isDESPurchase() && cargo.getCargoType() == CargoType.FOB) {
							final DetailConstraintStatusDecorator failure = new DetailConstraintStatusDecorator(
									(IConstraintStatus) ctx.createFailureStatus("Cargo|" + cargo.getLoadName() + " Cannot pair a DES Purchase to a FOB Sale."));
						...
						if (dischargeSlot.isFOBSale() && cargo.getCargoType() == CargoType.DES) {
							final DetailConstraintStatusDecorator failure = new DetailConstraintStatusDecorator(
									(IConstraintStatus) ctx.createFailureStatus("Cargo|" + cargo.getLoadName() + " Cannot pair a DES Purchase to a FOB Sale."));
