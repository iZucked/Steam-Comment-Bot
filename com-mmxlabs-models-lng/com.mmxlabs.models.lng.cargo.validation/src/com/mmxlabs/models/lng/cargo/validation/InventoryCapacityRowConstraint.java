...
public class InventoryCapacityRowConstraint extends AbstractModelMultiConstraint {
	...
	public void doValidate(final IValidationContext ctx, final IExtraValidationContext extraContext, final List<IStatus> failures) {
		...
		if (object instanceof InventoryCapacityRow icr) {

			if (icr.eContainer() instanceof Inventory inventory) {
				if (icr.getDate() == null) {
					final DetailConstraintStatusDecorator failure = new DetailConstraintStatusDecorator(
							(IConstraintStatus) ctx.createFailureStatus(String.format("Inventory model '%s': has an empty date in capacity row.", inventory.getName())));
				...
				if (icr.getMaxVolume() < icr.getMinVolume()) {
					final DetailConstraintStatusDecorator failure = new DetailConstraintStatusDecorator(
							(IConstraintStatus) ctx.createFailureStatus(String.format("Inventory model '%s': min capacity is greater than the max capacity!", inventory.getName())));
