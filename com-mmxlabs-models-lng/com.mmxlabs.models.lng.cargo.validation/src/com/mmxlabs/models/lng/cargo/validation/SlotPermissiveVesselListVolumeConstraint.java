...
/**
 * Validation constraint to check vessel capacity exceeds minimum amount of LNG
 * required for slot.
 */
public class SlotPermissiveVesselListVolumeConstraint extends AbstractModelMultiConstraint {
	...
	private DetailConstraintStatusDecorator createInvalidPermittedVesselStatus(final IValidationContext ctx, final Vessel vessel, final Slot<?> slot, int minRequiredVolumeM3) {
		...
		final String message = String.format("Permitted vessel %s can not service %s slot %s as vessel capacity %d m3 is less than required min volume for slot of %d m3", vessel.getName(), slotType,
				slot.getName(), vessel.getVesselOrDelegateCapacity(), minRequiredVolumeM3);
