...
public class FleetModelImporter implements ISubmodelImporter {
	...
	public UUIDObject importModel(final Map<String, CSVReader> inputs, final IImportContext context) {
		...
		if (inputs.containsKey(VESSEL_AVAILABILITY_KEY))
			fleetModel
					.getScenarioFleetModel()
					.getVesselAvailabilities()
					.addAll((Collection<? extends VesselAvailability>) vesselAvailabilityImporter.importObjects(FleetPackage.eINSTANCE.getVesselAvailability(), inputs.get(VESSEL_AVAILABILITY_KEY),
							context));
		...
		if (inputs.containsKey(EVENTS_KEY))
			fleetModel.getScenarioFleetModel().getVesselEvents()
	...
	public void exportModel(final MMXRootObject root, final UUIDObject model, final Map<String, Collection<Map<String, String>>> output) {
		...
		output.put(EVENTS_KEY, vesselEventImporter.exportObjects(fleetModel.getScenarioFleetModel().getVesselEvents(), root));
