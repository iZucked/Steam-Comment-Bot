		...
		if (target instanceof AbstractNominationSpec && !(target instanceof AbstractNomination)) {
			...
			if (refererId != null && !"".equals(refererId)) {
				...
				final DetailConstraintStatusDecorator status = new DetailConstraintStatusDecorator(
						(IConstraintStatus) ctx.createFailureStatus(String.format("Nomination specification with unknown contract name: \"%s\"", contractType, spec.getRefererId())));
				status.addEObjectAndFeature(spec, NominationsPackage.eINSTANCE.getAbstractNominationSpec_RefererId());
				statuses.add(status);
			} else {
				final DetailConstraintStatusDecorator status = new DetailConstraintStatusDecorator(
						(IConstraintStatus) ctx.createFailureStatus(String.format("Nomination specification with missing contract name.")));
				status.addEObjectAndFeature(spec, NominationsPackage.eINSTANCE.getAbstractNominationSpec_RefererId());
				statuses.add(status);
			}
		}
