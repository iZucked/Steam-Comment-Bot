...
/**
 * Validate dueDate + alertDate are prior to load/discharge date of associated
 * slot.
 */
					...
					if (slot != null) {
						...
						if (date != null && nomination.getDueDate() != null && date.isBefore(nomination.getDueDate())) {
							final DetailConstraintStatusDecorator status = new DetailConstraintStatusDecorator((IConstraintStatus) ctx
									.createFailureStatus(String.format("Nomination due date %s is after window start date (%s) of slot %s", nomination.getDueDate(), date, slot.getName())));
						...
						if (date != null && nomination.getAlertDate() != null && date.isBefore(nomination.getAlertDate())) {
							final DetailConstraintStatusDecorator status = new DetailConstraintStatusDecorator((IConstraintStatus) ctx
									.createFailureStatus(String.format("Nomination alert date %s is after window start date (%s) of slot %s", nomination.getAlertDate(), date, slot.getName())));
