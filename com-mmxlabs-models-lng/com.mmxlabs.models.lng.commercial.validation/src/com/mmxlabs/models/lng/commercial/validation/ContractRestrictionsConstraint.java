		...
		if (target instanceof Contract contract) {
			...
			if (contract.getRestrictedPorts().isEmpty() && contract.isRestrictedPortsArePermissive()) {
				final DetailConstraintStatusDecorator status = new DetailConstraintStatusDecorator(
						(IConstraintStatus) ctx.createFailureStatus(String.format("Contract %s has an empty list of allowed ports.", contract.getName())));
			...
			if (contract.getRestrictedContracts().isEmpty() && contract.isRestrictedContractsArePermissive()) {
				final DetailConstraintStatusDecorator status = new DetailConstraintStatusDecorator(
						(IConstraintStatus) ctx.createFailureStatus(String.format("Contract %s has an empty list of allowed contracts.", contract.getName())));
			...
			if (contract.getRestrictedVessels().isEmpty() && contract.isRestrictedVesselsArePermissive()) {
				final DetailConstraintStatusDecorator status = new DetailConstraintStatusDecorator(
						(IConstraintStatus) ctx.createFailureStatus(String.format("Contract %s has an empty list of allowed vessels.", contract.getName())));
