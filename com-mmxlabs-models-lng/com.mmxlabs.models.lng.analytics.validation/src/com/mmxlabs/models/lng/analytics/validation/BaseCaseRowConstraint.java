	...
	protected String validate(@NonNull final IValidationContext ctx, @NonNull final IExtraValidationContext extraContext, @NonNull final List<IStatus> statuses) {
		...
		if (target instanceof BaseCaseRow) {
			...
			if (nonShipped == ShippingType.Shipped) {
				...
				final DetailConstraintStatusDecorator deco = new DetailConstraintStatusDecorator((IConstraintStatus) ctx.createFailureStatus("Sandbox|Starting point: incompatible slot combination."));
			...
			if (baseCaseRow.getVesselEventOption() instanceof CharterOutOpportunity && AnalyticsBuilder.isRoundTripOption(baseCaseRow.getShipping())) {
