	...
	protected String validate(@NonNull final IValidationContext ctx, @NonNull final IExtraValidationContext extraContext, @NonNull final List<IStatus> statuses) {
		...
		if (target instanceof BaseCaseRow) {
			...
			if (nonShipped == ShippingType.Shipped) {
				if (baseCaseRow.getBuyOption() != null && baseCaseRow.getSellOption() != null) {
					if (baseCaseRow.getShipping() == null) {
						final DetailConstraintStatusDecorator deco = new DetailConstraintStatusDecorator((IConstraintStatus) ctx.createFailureStatus("Target - no shipping option defined."));
				...
				final DetailConstraintStatusDecorator deco = new DetailConstraintStatusDecorator((IConstraintStatus) ctx.createFailureStatus("Target - incompatible slot combination."));
			...
			if (baseCaseRow.getShipping() != null && baseCaseRow.getShipping().eContainer() == null) {
				final DetailConstraintStatusDecorator deco = new DetailConstraintStatusDecorator((IConstraintStatus) ctx.createFailureStatus("Target - uncontained shipping"));
				deco.addEObjectAndFeature(baseCaseRow, AnalyticsPackage.Literals.BASE_CASE_ROW__SHIPPING);
				statuses.add(deco);
			}
			...
			if (baseCaseRow.getShipping() != null && !SandboxConstraintUtils.vesselRestrictionsValid(baseCaseRow.getBuyOption(), baseCaseRow.getSellOption(), baseCaseRow.getShipping())) {
				final DetailConstraintStatusDecorator deco = new DetailConstraintStatusDecorator((IConstraintStatus) ctx.createFailureStatus("Target - load not permitted with this shipping option"));
				deco.addEObjectAndFeature(baseCaseRow, AnalyticsPackage.Literals.BASE_CASE_ROW__SHIPPING);
				statuses.add(deco);
			}
