 ...
 * 
 * Several tests are currently commented out, as the @Disabled annotation did not prevent them from being run, for some reason.
 */
public class RollForwardEngineTest {
	 ...
	 * 
	 * @param isLoadSlot
	 *            Whether to create a load slot (a discharge slot is created otherwise)
	 * @param cargoModel
	 *            The data model to create the slot in
	 * @param name
	 *            The name to give the slot
	 * @param date
	 *            The date that the slot is created relative to
	 * @param daysEarlierForStart
	 *            The number of days the slot start time occurs earlier than the specified date (can be negative)
	...
	/**
	 * Helper class to provide unique names by appending consecutive numbers to successive usages of the same name. Will not work properly if names are used that already have a numeric suffix.
	 * 
	 * @author simonmcgregor
	 *
	 */
	...
	/**
	 * Creates a new slot in the specified time relation to the particular date. Uses the slot type, the time relation, and the name of the date to generate a name for the slot.
	 * 
	 * @param isLoadSlot
	 * @param cargoModel
	 * @param timeRelation
	...
	/**
	 * The following method is obsolete, since we no longer remove data when we roll forward, but should be retained in case we want to reinstate the removal logic.
	 * 
	 * @return
	 */
	 ...
	 * @Test
	 * 
	 * @Disabled public void examineSlotsAndCargoesTest() { SlotCargoModelTestData testData = generateSlotCargoDataWithRemovals();
	 * 
	 * final RollForwardEngine engine = new RollForwardEngine();
	 * 
	 * final List<Slot<?>> slotsToRemove = new LinkedList<>(); final List<Slot<?>> slotsToFreeze = new LinkedList<>(); final List<Cargo> cargoesToRemove = new LinkedList<>(); final List<Cargo>
	 * cargoesToFreeze = new LinkedList<>();
	 * 
	 ...
	 * 
	 * // Check lists contain expected items Assert.assertEquals(testData.slotsExpectedToBeRemoved, new HashSet<>(slotsToRemove)); Assert.assertEquals(testData.slotsExpectedToBeFrozen, new
	 * HashSet<>(slotsToFreeze)); Assert.assertEquals(testData.cargoesExpectedToBeRemoved, new HashSet<>(cargoesToRemove)); Assert.assertEquals(testData.cargoesExpectedToBeFrozen, new
	 * HashSet<>(cargoesToFreeze)); }
	 */
	...
	public void slotsAndCargoesToFreezeTest() {
		...
		Assertions.assertEquals(slotCargoTestData.slotsExpectedToBeFrozen, new HashSet<>(slotsToFreeze), "Expected slots are not frozen.");
		Assertions.assertEquals(slotCargoTestData.cargoesExpectedToBeFrozen, new HashSet<>(cargoesToFreeze), "Expected cargoes are not frozen.");
	}
	...
	public void eventsToFreezeTest() {
		...
		// Check lists contain expected items
		Assertions.assertEquals(vesselEventTestData.eventsExpectedToBeFrozen, new HashSet<>(eventsToFreeze), "Expected events are not frozen.");
	}
	 ...
	 * @Test
	 * 
	 * @Disabled public void examineVesselEventsTest() {
	 * 
	 * VesselEventTestData testData = generateVesselEventTestDataWithRemoval();
	 * 
	 * final RollForwardEngine engine = new RollForwardEngine();
	 * 
	 * final List<VesselEvent> eventsToRemove = new LinkedList<>(); final List<VesselEvent> eventsToFreeze = new LinkedList<>();
	 * 
	 ...
	 * 
	 * // Check lists contain expected items Assert.assertEquals(testData.eventsExpectedToBeRemoved, new HashSet<>(eventsToRemove)); Assert.assertEquals(testData.eventsExpectedToBeFrozen, new
	 * HashSet<>(eventsToFreeze)); }
	 */
	 ...
	 * @Test
	 * 
	 * @Disabled // removal logic is obsolete public void generateCargoAndEventChangesTest_SlotToRemove() {
	 * 
	 ...
	 * 
	 * Assert.assertEquals(1, changes.size()); final IRollForwardChange change = changes.get(0);
	 * 
	 * Assert.assertTrue(change instanceof RemoveSlotChange);
	 * 
	 * final RemoveSlotChange removeSlotChange = (RemoveSlotChange) change; Assert.assertSame(slot, removeSlotChange.getChangedObject()); }
	 */

	/*
	 * @Test
	 * 
	 * @Disabled // uses obsolete removals logic public void generateCargoAndEventChangesTest_LoadSlotToFreeze_Old() {
	 * 
	 ...
	 * 
	 * // We do not freeze slots yet Assert.assertEquals(0, changes.size()); }
	 */

	/*
	 * @Test public void generateCargoAndEventChangesTest_LoadSlotToFreeze() {
	 * 
	 * final LoadSlot loadSlot = CargoFactory.eINSTANCE.createLoadSlot();
	 * 
	 ...
	 * 
	 * final FreezeSlotChange removeSlotChange = (FreezeSlotChange) change; Assert.assertSame(loadSlot, removeSlotChange.getChangedObject()); }
	 */

	/*
	 * @Test
	 * 
	 * @Disabled // uses obsolete removals code public void generateCargoAndEventChangesTest_EventToRemove() {
	 * 
	 * final VesselEvent event = CargoFactory.eINSTANCE.createDryDockEvent();
	 * 
	 ...
	 * 
	 * final RemoveVesselEventChange removeSlotChange = (RemoveVesselEventChange) change; Assert.assertSame(event, removeSlotChange.getChangedObject()); }
	 */

	/*
	 * @Test
	 * 
	 * @Disabled // uses obsolete code with removals logic public void generateCargoAndEventChangesTest_EventToFreeze_Old() {
	 * 
	 * final VesselEvent event = CargoFactory.eINSTANCE.createDryDockEvent();
	 * 
	 ...
	 * 
	 * Port p = PortFactory.eINSTANCE.createPort(); Location l = PortFactory.eINSTANCE.createLocation(); l.setTimeZone("UTC"); p.setLocation(l); event.setPort(p);
	 * 
	 ...
	 * 
	 * final VesselEventVisit vesselEventVisit = ScheduleFactory.eINSTANCE.createVesselEventVisit(); vesselEventVisit.setVesselEvent(event);
	 * 
	 * sequence.getEvents().add(vesselEventVisit);
	 * 
	 * LocalDate dt = LocalDate.of(2014, 9, 4);
	 * 
	 * // Create 10 day window, with schedule start halfway event.setStartAfter(dt.atTime(0, 0)); dt = dt.plusDays(5); vesselEventVisit.setStart(dt.atStartOfDay(ZoneId.of("UTC"))); dt =
	 * dt.plusDays(1); vesselEventVisit.setEnd(dt.atStartOfDay(ZoneId.of("UTC"))); dt = dt.plusDays(4); event.setStartAfter(dt.atTime(0, 0));
	 * 
	 ...
	 * 
	 * FreezeVesselEventChange freezeChange = null; FixVesselEventWindowChange windowChange = null; for (final IRollForwardChange change : changes) { if (change instanceof FreezeVesselEventChange) {
	 * Assert.assertNull("Only one freeze change expected", freezeChange); freezeChange = (FreezeVesselEventChange) change; } else if (change instanceof FixVesselEventWindowChange) {
	 * Assert.assertNull("Only one window change expected", windowChange); windowChange = (FixVesselEventWindowChange) change; } else { Assert.fail("Unexpected change"); } } }
	 */
	...
	public void generateCargoAndEventChangesTest_EventToFreeze() {
		...
		for (final IRollForwardChange change : changes) {
			if (change instanceof FreezeVesselEventChange) {
				Assertions.assertNull(freezeChange, "Only one freeze change expected");
				freezeChange = (FreezeVesselEventChange) change;
			} else if (change instanceof FixVesselEventWindowChange) {
				Assertions.assertNull(windowChange, "Only one window change expected");
				windowChange = (FixVesselEventWindowChange) change;
			} else {
				Assertions.fail("Unexpected change");
			}
		}
	}
	 ...
	 * @Test
	 * 
	 * @Disabled // uses obsolete code with removals logic public void generateCargoAndEventChangesTest_CargoToFreeze_Old() {
	 * 
	 * final Cargo cargo = CargoFactory.eINSTANCE.createCargo(); final LoadSlot loadSlot = CargoFactory.eINSTANCE.createLoadSlot(); final DischargeSlot dischargeSlot =
	 * CargoFactory.eINSTANCE.createDischargeSlot();
	 * 
	 * Port p = PortFactory.eINSTANCE.createPort(); Location l = PortFactory.eINSTANCE.createLocation(); l.setTimeZone("UTC"); p.setLocation(l); loadSlot.setPort(p); dischargeSlot.setPort(p);
	 * 
	 ...
	 * 
	 * final CargoAllocation cargoAllocation = ScheduleFactory.eINSTANCE.createCargoAllocation(); { schedule.getCargoAllocations().add(cargoAllocation); } {
	 * 
	 * final Slot slot = loadSlot;
	 * 
	 * final SlotAllocation slotAllocation = ScheduleFactory.eINSTANCE.createSlotAllocation(); slotAllocation.setSlot(slot); cargoAllocation.getSlotAllocations().add(slotAllocation);
	 * 
	 * final SlotVisit slotVisit = ScheduleFactory.eINSTANCE.createSlotVisit(); slotVisit.setSlotAllocation(slotAllocation);
	 * 
	 * schedule.getSlotAllocations().add(slotAllocation); sequence.getEvents().add(slotVisit);
	 * 
	 * slot.setWindowStart(dt); slot.setWindowSize(48);
	 * 
	 * dt = dt.plusDays(1); slotVisit.setStart(dt.atStartOfDay(ZoneId.of("UTC"))); slotVisit.setEnd(dt.atStartOfDay(ZoneId.of("UTC"))); } dt = dt.plusDays(10); {
	 * 
	 ...
	 * slot.setWindowStart(dt); slot.setWindowSize(48);
	 * 
	 * dt = dt.plusDays(1); slotVisit.setStart(dt.atStartOfDay(ZoneId.of("UTC"))); slotVisit.setEnd(dt.atStartOfDay(ZoneId.of("UTC"))); }
	 * 
	 ...
	 * 
	 * FreezeCargoChange freezeChange = null; FixSlotWindowChange windowChange1 = null; FixSlotWindowChange windowChange2 = null; for (final IRollForwardChange change : changes) { if (change
	 * instanceof FreezeCargoChange) { Assert.assertNull("Only one freeze change expected", freezeChange); freezeChange = (FreezeCargoChange) change; } else if (change instanceof FixSlotWindowChange)
	 * { if (windowChange1 == null) { windowChange1 = (FixSlotWindowChange) change; } else if (windowChange2 == null) { windowChange2 = (FixSlotWindowChange) change; } else {
	 * Assert.fail("Only two window changes expected"); } } else { Assert.fail("Unexpected change"); } } }
	 */
		...
		for (final IRollForwardChange change : changes) {
			if (change instanceof FreezeCargoChange) {
				...
				if (windowChange1 == null) {
					...
					Assertions.fail("Only two window changes expected");
				}
			} else {
				Assertions.fail("Unexpected change");
			}
	 ...
	 * @Test
	 * 
	 * @Disabled // uses obsolete code with removals logic public void generateCargoAndEventChangesTest_CargoToRemove() {
	 * 
	 * final Cargo cargo = CargoFactory.eINSTANCE.createCargo(); final LoadSlot loadSlot = CargoFactory.eINSTANCE.createLoadSlot(); final DischargeSlot dischargeSlot =
	 * CargoFactory.eINSTANCE.createDischargeSlot();
	 * 
	 ...
	 * Assert.assertEquals(3, changes.size());
	 * 
	 * RemoveSlotChange removeChange1 = null; RemoveSlotChange removeChange2 = null; RemoveCargoChange removeCargo = null; for (final IRollForwardChange change : changes) { if (change instanceof
	 * RemoveCargoChange) { Assert.assertNull("Only one remove cargo change expected", removeCargo); removeCargo = (RemoveCargoChange) change; } else if (change instanceof RemoveSlotChange) { if
	 * (removeChange1 == null) { removeChange1 = (RemoveSlotChange) change; } else if (removeChange2 == null) { removeChange2 = (RemoveSlotChange) change; } else {
	 * Assert.fail("Only two remove slot changes expected"); } } else { Assert.fail("Unexpected change"); } } Assert.assertNotNull(removeCargo); Assert.assertNotNull(removeChange1);
