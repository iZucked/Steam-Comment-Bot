 ...
 * @since 2.0.0
 */
public class ResultSorterColumnHeaderRenderer extends GridHeaderRenderer {
	...
	public Point computeSize(GC gc, int wHint, int hHint, Object value) {
		...
		// if (parent.getColumn(count - 1) != column) {
		// columnText = columnText += ">>";
		// }
		x += gc.stringExtent(columnText).x + rightMargin;
		// } else {
		// int plainTextWidth;
	...
	public void paint(GC gc, Object value) {
		...
		// Grid parent = column.getParent();
		// int count = parent.getColumnCount();
		// if (false && parent.getColumn(count - 1) != column) {
		// int y_margin = 5;
		// int x_size = 5;
		// int l = gc.getLineWidth();
		// gc.setLineWidth(2);
		// gc.drawLine(getBounds().x + getBounds().width - x_size - 1, getBounds().y + y_margin, getBounds().x + getBounds().width - 1, getBounds().y + getBounds().height / 2);
		// gc.drawLine(getBounds().x + getBounds().width - 1, getBounds().y + getBounds().height / 2, getBounds().x + getBounds().width - x_size - 1, getBounds().y + getBounds().height - y_margin);
		// gc.setLineWidth(l);
		// }
