...
public class CommodityCurveOptionsComponent extends AbstractSandboxComponent<Object, AbstractAnalysisModel> {

	private GridTreeViewer optionsViewer;
	private MenuManager mgr;

	public CommodityCurveOptionsComponent(final @NonNull IScenarioEditingLocation scenarioEditingLocation, final Map<Object, IStatus> validationErrors,
	...
	public void createControls(Composite parent, boolean expanded, IExpansionListener expansionListener, Object optionModellerView) {
		ExpandableComposite expandable = wrapInExpandable(parent, "Commodity Curves", this::createOptionsViewer, expandableComposite -> {
...
//			}
			...
			c.addMouseListener(OptionMenuHelper.createNewCommodityCurveOptionMenuListener(c.getParent(), scenarioEditingLocation, modelProvider));
		}, false);
	...
	private Control createOptionsViewer(final Composite buyComposite) {
		...
		CellFormatterLabelProvider labelProvider = new CommodityCurveLabelProvider(sandboxUIHelper, new CommodityCurveOptionDescriptionFormatter(), validationErrors, "Commodity Curves");
		createColumn_TempForNebulaBugFix(optionsViewer, labelProvider, "Events", false);

		optionsViewer.setContentProvider(new OptionsViewerContentProvider(AnalyticsPackage.Literals.ABSTRACT_ANALYSIS_MODEL__COMMODITY_CURVES));
		hookOpenEditor(optionsViewer);
		{
			mgr = new MenuManager();
			final CommodityCurveOptionsContextMenuManager listener = new CommodityCurveOptionsContextMenuManager(optionsViewer, scenarioEditingLocation, mgr);
			inputWants.add(listener::setModel);
