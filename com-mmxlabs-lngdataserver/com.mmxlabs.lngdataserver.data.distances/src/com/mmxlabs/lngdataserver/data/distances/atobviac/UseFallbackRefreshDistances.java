 ...
 *
 */
public class UseFallbackRefreshDistances {
	...
	public static void gatherNew(String destData) throws Exception {
		...
		URL destPortJsonURL = UseFallbackRefreshDistances.class.getResource("/" + destData + "/ports.json");
...
//						existingPortsSet.add(l.getUpstreamID());
						if (l.getFallbackUpstreamID() != null) {
							...
							fallbackMapping.put(l.getUpstreamID(), l.getFallbackUpstreamID());
						}
					}
				});
					...
					try (InputStream fis = new BufferedInputStream(new FileInputStream(cacheFile))) {
						...
						try (JsonParser jp = jf.createParser(fis)) {
							...
							while (jp.hasCurrentToken()) {
								...
								if (r.getErrorCode() != null && r.getErrorCode().startsWith("Voyage contains unreviewed Voyage Leg")) {
									String a = fallbackMapping.getOrDefault(r.getFrom(), r.getFrom());
									String b = fallbackMapping.getOrDefault(r.getTo(), r.getTo());
									lookups.add(Pair.of(a,b));
								} else {
								}
