...
public class VesselsIO {
	...
	public static VesselsVersion read(final InputStream is) throws IOException {
		...
		try (JsonParser jp = jf.createParser(is)) {
			...
			if (header.getModelVersion() == CURRENT_MODEL_VERSION) {
				return jp.readValueAs(VesselsVersion.class);
			} else {
	...
	public static String write(final VesselsVersion version) throws IOException {
		final ObjectMapper mapper = createObjectMapper();
