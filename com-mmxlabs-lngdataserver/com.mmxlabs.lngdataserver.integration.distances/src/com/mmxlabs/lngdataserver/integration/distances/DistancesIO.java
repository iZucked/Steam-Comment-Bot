...
public class DistancesIO {
	...
	public static DistancesVersion read(final InputStream is) throws IOException {
		...
		try (JsonParser jp = jf.createParser(is)) {
			...
			if (header.getModelVersion() == CURRENT_MODEL_VERSION) {
				return jp.readValueAs(DistancesVersion.class);
			} else {
	...
	public static void write(final DistancesVersion version, final OutputStream os) throws IOException {
	...
	public static String write(final DistancesVersion version) throws IOException {
		final ObjectMapper mapper = createObjectMapper();
