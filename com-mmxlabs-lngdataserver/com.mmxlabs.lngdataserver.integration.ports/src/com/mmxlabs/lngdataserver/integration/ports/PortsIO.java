...
public class PortsIO {
	...
	public static PortsVersion read(final InputStream is) throws Exception {
		...
		try (JsonParser jp = jf.createParser(is)) {
			...
			if (header.getModelVersion() == CURRENT_MODEL_VERSION) {
				return jp.readValueAs(PortsVersion.class);
			} else {
	...
	public static void write(final PortsVersion version, final OutputStream os) throws IOException {
	...
	public static String write(final PortsVersion version) throws IOException {
		final ObjectMapper mapper = createObjectMapper();
