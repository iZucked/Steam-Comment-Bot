...
public class DefaultPortProviderTests {
	...
	public void testApproxName() throws AuthenticationException, ClientProtocolException, IOException, ParseException {
		...
		samplePorts.add(p5);
		assertEquals("Skikda", DefaultPortProvider.staticGetPortByApproxName(samplePorts, "Skikda").getName());
		assertEquals("Skikda", DefaultPortProvider.staticGetPortByApproxName(samplePorts, "Skikda").getName());
		assertEquals("Arzew", DefaultPortProvider.staticGetPortByApproxName(samplePorts, "Arzev").getName());
	}
