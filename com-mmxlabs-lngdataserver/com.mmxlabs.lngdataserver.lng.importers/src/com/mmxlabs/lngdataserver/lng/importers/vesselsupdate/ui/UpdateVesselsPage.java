...
public class UpdateVesselsPage extends WizardPage {
	...
	protected UpdateVesselsPage(final String pageName, ModelReference modelReference, VesselsVersion vesselRecords) {
		super(pageName);
	...
	private boolean doVesselUpdateSteps() {
		try {
			getContainer().run(true, false, new IRunnableWithProgress() {
				@Override
				public void run(final IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
					monitor.beginTask("Apply vessel update steps", IProgressMonitor.UNKNOWN);
					try {
						{
							final CompoundCommand command = new CompoundCommand("Update vessels");
							// Apply basic updates
							steps.stream() //
	...
	private List<UpdateItem> generatePortUpdateSteps() {
		try {
			...
			getContainer().run(true, false, new IRunnableWithProgress() {
				@Override
				public void run(final IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
					monitor.beginTask("Generate update vessel steps", IProgressMonitor.UNKNOWN);
					try {
						...
						lsteps.addAll(VesselsToUpdateItems.generateUpdates(editingDomain, portModel, fleetModel, vesselRecords));
