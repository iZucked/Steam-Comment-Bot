...
public class VesselsClient {

	private static final Logger LOGGER = LoggerFactory.getLogger(VesselsClient.class);
	private static final OkHttpClient CLIENT = new OkHttpClient();
	public static final MediaType JSON = MediaType.parse("application/json; charset=utf-8");

	public static boolean saveVersion(String baseUrl, Version version) throws IOException {
		String json = new ObjectMapper().writeValueAsString(version.getVessels());

		RequestBody body = RequestBody.create(JSON, json);
		Request request = new Request.Builder().url(baseUrl + "/vessel/bulk").post(body).build();
