...
public class PeriodVerifierUtil {
	...
	public static void runTest(final LNGScenarioRunner scenarioRunner, final boolean checkPanamaBookings) {
		...
		final LNGScenarioModel fullModel = scenarioRunner.getScenarioToOptimiserBridge().getScenario();
		final LNGScenarioModel periodModel = scenarioRunner.getScenarioToOptimiserBridge().getOptimiserScenario();
		...
		for (final Cargo cargo : periodCargoModel.getCargoes()) {
			...
			for (int idx = 0; idx < numberOfEvents; ++idx) {
				...
				if (fullEvent instanceof Journey) {
					...
					Assert.assertEquals(fullJourney.getRoute().getRouteOption(), periodJourney.getRoute().getRouteOption());
