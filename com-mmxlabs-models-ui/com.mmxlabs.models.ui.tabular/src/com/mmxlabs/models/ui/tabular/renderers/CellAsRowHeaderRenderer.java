...
public class CellAsRowHeaderRenderer extends GridCellRenderer {
...
//			gc.setForeground(foreground == null ? getDisplay().getSystemColor(SWT.COLOR_WIDGET_FOREGROUND) : foreground);
		...
		if (isCheck()) {
			...
			if (isCenteredCheckBoxOnly(item)) {
				...
				checkRenderer.setBounds(getBounds().x + ((getBounds().width - checkRenderer.getBounds().width) / 2), (getBounds().height - checkRenderer.getBounds().height) / 2 + getBounds().y,
						checkRenderer.getBounds().width, checkRenderer.getBounds().height);
			} else {
				checkRenderer.setBounds(getBounds().x + x, (getBounds().height - checkRenderer.getBounds().height) / 2 + getBounds().y, checkRenderer.getBounds().width,
		...
		if (drawAsSelected) {
			boolean hasFocus = item.getParent().isFocusOnGrid();
//			Color backgroundColor = hasFocus ? getDisplay().getSystemColor(SWT.COLOR_LIST_SELECTION) : getDisplay().getSystemColor(SWT.COLOR_WIDGET_LIGHT_SHADOW);
			Color backgroundColor = hasFocus ? getDisplay().getSystemColor(SWT.COLOR_LIST_SELECTION) : getDisplay().getSystemColor(SWT.COLOR_LIST_SELECTION);
			Color foregroundColor = hasFocus ? getDisplay().getSystemColor(SWT.COLOR_LIST_SELECTION_TEXT) : getDisplay().getSystemColor(SWT.COLOR_BLACK);
			gc.setBackground(backgroundColor);
			gc.setForeground(foregroundColor);
		} else {
...
//		}
	...
	protected boolean isRowSelected(final GridItem item) {
		final Grid grid = item.getParent();
		final GridItem[] selectedItems = grid.getSelection();
		if (selectedItems.length == 1) {
			final GridItem selectedItem = selectedItems[0];
			return item.getRowIndex() == selectedItem.getRowIndex();
		}
		return false;
	}
