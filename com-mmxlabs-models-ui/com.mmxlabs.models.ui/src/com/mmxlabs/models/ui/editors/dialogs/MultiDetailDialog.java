...
public abstract class MultiDetailDialog extends Dialog {
	...
	private IInlineEditorWrapper wrapper;

	public MultiDetailDialog(final Shell parentShell, final IReferenceValueProviderProvider valueProviderProvider, 
			final EditingDomain editingDomain) {
		super(parentShell);
		
		wrapper = new IInlineEditorWrapper() {
			@Override
			public IInlineEditor wrap(final IInlineEditor proxy) {
				...
				return new IInlineEditor() {
					...
					public Control createControl(final Composite parent) {
						...
						final Pair<EMFPath, EStructuralFeature> pair = 
								new Pair<EMFPath, EStructuralFeature>(getPath(), getFeature());
	...
	private void displayProxy() {
		displayComposite = Activator.getDefault().getDisplayCompositeFactoryRegistry().getDisplayCompositeFactory(proxy.eClass()).createToplevelComposite((Composite) getDialogArea(), proxy.eClass());
		
		displayComposite.setEditorWrapper(wrapper);
		
		displayComposite.display(root, proxy);
		disableControls();
	}
	...
	/**
	 * This maps from objects contained in the proxy object to paths to the sub-objects.
	 */
	private final Map<EObject, EMFPath> proxyPaths = new HashMap<EObject, EMFPath>();
