...
public class XmlStreamWriter {
	...
	private final XMLOutputFactory outputFactory = XMLOutputFactory
			.newFactory();
	private XMLEventWriter eventWriter;

	private ITypeConvertorService typeConvertor = new TypeConvertorService();

	private String namespace = "http://www.mmxlabs.com/schemas/InterfaceRecorder-1.0.0";
	 ...
	 */
	public void writeHeader(Class<?> interfaceClass) throws XMLStreamException {
		...
		final StartElement startOperations = eventFactory.createStartElement(
				prefix, namespace, XmlRecordingConstants.ELEMENT_OPERATIONS);
		eventWriter.add(startOperations);

		eventWriter.add(eventFactory.createNamespace("xsi",
				"http://www.w3.org/2001/XMLSchema-instance"));
		eventWriter.add(eventFactory.createAttribute("xsi:schemaLocation",
				namespace + " schemas/InterfaceRecorder-1.0.0.xsd"));
		...
		eventWriter.add(eventFactory.createAttribute(
				XmlRecordingConstants.ATTR_OPERATIONS_INTERFACE, interfaceClass
						.getCanonicalName()));
	}
	...
	public void recordMethod(final Method m, final Object[] args,
		...
		final StartElement methodStart = eventFactory.createStartElement(
				prefix, namespace, XmlRecordingConstants.ELEMENT_METHOD);
		eventWriter.add(methodStart);

		eventWriter.add(eventFactory.createAttribute(
				XmlRecordingConstants.ATTR_METHOD_NAME, m.getName()));

		if (m.getReturnType() != null
				&& !m.getReturnType().getName().equals("void")) {
			final String refName = "reference-" + counter.getAndIncrement();
			eventWriter.add(eventFactory.createAttribute(
					XmlRecordingConstants.ATTR_METHOD_RETURNED_REF, refName));
		...
		final EndElement endElement = eventFactory.createEndElement(prefix,
				namespace, XmlRecordingConstants.ELEMENT_METHOD);

		eventWriter.add(endElement);
	}

	private void writeArgument(final Object arg, final Class<?> paramType)
			throws XMLStreamException {
		eventWriter.add(eventFactory.createStartElement(prefix, namespace,
				XmlRecordingConstants.ELEMENT_ARGUMENT));

		eventWriter.add(eventFactory.createAttribute(
				XmlRecordingConstants.ATTR_ARGUMENT_TYPE, paramType
						.getCanonicalName()));

		if (objectMap.containsKey(arg)) {
			final String ref = objectMap.get(arg);
			eventWriter.add(eventFactory.createAttribute(
					XmlRecordingConstants.ATTR_ARGUMENT_REF, ref));
		} else {

			eventWriter.add(eventFactory.createAttribute(
					XmlRecordingConstants.ATTR_ARGUMENT_VALUE, typeConvertor
							.toString(paramType.getCanonicalName(), arg)));
		}
		eventWriter.add(eventFactory.createEndElement(prefix, namespace,
				XmlRecordingConstants.ELEMENT_ARGUMENT));
	}
	...
	void writeFooter() throws XMLStreamException {
		...
		final EndElement endOperations = eventFactory.createEndElement(prefix,
				namespace, XmlRecordingConstants.ELEMENT_OPERATIONS);
