...
public class ArithmeticParser extends ExpressionParser<Double> {
	...
	public ArithmeticParser() {
		setPrefixOperatorFactory(
				new IPrefixOperatorFactory<Double>() {
					...
					@Override
					public IExpression<Double> createPrefixOperator(char operator,
							IExpression<Double> argument) {
		...
		setInfixOperatorFactory(new IInfixOperatorFactory<Double>() {
			...
			@Override
			public IExpression<Double> createInfixOperator(char operator,
					IExpression<Double> lhs, IExpression<Double> rhs) {
		...
		setFunctionFactory(new IFunctionFactory<Double>() {
			...
			@Override
			public IExpression<Double> createFunction(String name,
					List<IExpression<Double>> arguments) {
				if (name.equals("avg")) {
