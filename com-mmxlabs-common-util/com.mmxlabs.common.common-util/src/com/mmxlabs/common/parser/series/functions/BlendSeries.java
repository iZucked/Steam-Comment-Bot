...
public class BlendSeries implements ISeries {
	...
	public BlendSeries(final @NonNull ISeries targetSeries, @NonNull final ISeries tier1Series, final double threshold, @NonNull final ISeries tier2Series) {
	...
	public Number evaluate(final int timePoint, final Map<String, String> params) {
		...
		final ExprSelector selected = BlendASTNode.select(inputVolume, threshold);
		switch (selected) {
		case LOW:
			return tier1Series.evaluate(timePoint, params);
		case BLEND:
