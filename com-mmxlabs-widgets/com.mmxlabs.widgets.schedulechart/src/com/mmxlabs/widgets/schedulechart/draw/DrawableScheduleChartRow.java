...
public class DrawableScheduleChartRow extends DrawableElement {
	...
	protected List<BasicDrawableElement> getBasicDrawableElements(Rectangle bounds, DrawerQueryResolver queryResolver) {
		...
		// Add background
		res.add(BasicDrawableElements.Rectangle.withBounds(bounds.x, bounds.y, bounds.width, ROW_HEIGHT).bgColour(rowNum % 2 == 0 ? null : ALT_BG_COLOUR).strokeColour(GRAY).alpha(160).create());
