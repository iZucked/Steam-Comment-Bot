...
public class GanttSpecialDateRange {
    ...
    public GanttSpecialDateRange(GanttChart parent, Calendar start, Calendar end) {
        _recurDays = new ArrayList<Integer>();
        _parentChart = parent;
        if (parent != null) {
    ...
    List<List<Calendar>> getBlocks(Calendar start, Calendar end) {
        if (_cachedRanges != null) {
            return _cachedRanges;
        }
        
        _cachedRanges = new ArrayList<List<Calendar>>();
        ...
        for (int i = 0; i < _recurCount; i++) {
            ...
            for (int x = 0; x < _recurDays.size(); x++) {
                ...
                List<Calendar> foo = new ArrayList<Calendar>();
                foo.add((Calendar)cal.clone());
                foo.add((Calendar)calEnd.clone());
