 ...
 * individually to be locked, non-movable, non-resizable and much more.
 * <p>
 ...
 *         <a href="mailto:emil.crumhorn@gmail.com">emil.crumhorn@gmail.com</a>
 */
public class GanttEvent extends AbstractGanttEvent implements IGanttChartItem, Cloneable {

	/*
	 * public static final int TYPE_EVENT = 0; public static final int
	 * TYPE_CHECKPOINT = 1; public static final int TYPE_IMAGE = 2; public static
	 * final int TYPE_SCOPE = 3;
	 */
	 ...
	 */
	public GanttEvent(final GanttChart parent, final Object data, final String name, final Calendar startDate, final Calendar endDate, final Calendar revisedStart, final Calendar revisedEnd,
	...
	/**
	 * Returns whatever is the earliest calendar of the start date and the actual
	 * start date. If any of them are null, whichever has a calendar is returned. If
	 * both are null, null is returned.
	 * 
	 * @return Earliest start date
	 */
	public Calendar getEarliestStartDate() {
		if (_revisedStart == null && _startDate == null) {
			return null;
		}
	...
	/**
	 * Returns whatever is the latest calendar of the end date and the actual end
	 * date. If any of them are null, whichever has a calendar is returned. If both
	 * are null, null is returned.
	 * 
	 * @return Latest end date
	 */
	public Calendar getLatestEndDate() {
		if (_revisedEnd == null && _endDate == null) {
			return null;
		}
	...
	public void setStartDate(final Calendar startDate) {
		...
		if (_noMoveBeforeDate != null && startDate.before(_noMoveBeforeDate)) {
			return;
		}
	...
	/**
	 * Forces the chart to recognize that something within this event has changed
	 * and that it needs an update. This method will cause a redraw if told to
	 * redraw.
	 * 
	...
	public void setEndDate(final Calendar endDate) {
		...
		if (_noMoveAfterDate != null && endDate.after(_noMoveAfterDate)) {
			return;
		}
	 ...
	 * created. Custom events will be mixed with built-in events (if they exist).
	 * The menu will be created on the first get call to this method, any subsequent
	 * method calls will return the same menu object. Do remember that a menu counts
	 * as a handle towards the maximum number of handles. If you have very many
	 * events, it is probably better to use a different way of creating menus, this
	 * is merely meant as a convenience.
	 * 
	 * @return Menu
	 */
	public Menu getMenu() {
...
public	void setBounds(final Rectangle bounds) {
	...
	/**
	 * Sets new revised dates. This is useful when you need to update two dates that
	 * move at the same time (such as manually doing a move via setDates). Normally
	 * each setting of a date would check it against its start date or end date to
	 * make sure it doesn't overlap. This does too, but at the same time, thus, no
	 * oddity in movement will appear. This is rather difficult to explain, but if
	 * you experience event-length changes when using individual start and end date
	 * sets that appear at the same time, you probably want to use this method
	 * instead.
	 * <p>
	...
	private void setRevisedDates(final Calendar revisedStart, final Calendar revisedEnd) {
		...
		if (revisedStart != null && getActualEndDate() != null && revisedStart.after(getActualEndDate())) {
			return;
		}

		if (revisedEnd != null && getActualStartDate() != null && revisedEnd.before(getActualStartDate())) {
			return;
		}
	 ...
	 * always has to be set before the other. So when you set a new end and start
	 * date one is checked for overlap before the other and is thus ignored.
	 * Sometimes you just want to set the dates and not enforce validation. Do
	 * remember, you need to validate the dates yourself before setting them. If the
	 * start comes after the end or vice versa you will run into serious drawing
	 * issues and strange behavior.
	 * 
	 * @param revisedStart New revised start date
	 * @param validate     true if to validate the date as normal, false to just set
	 *                     the date as is.
	 */
	 ...
	 * which order the new dates should be set. If you say left to right, the start
	 * is set first, the end last.
	 * 
	 * @param revisedStart Revised start date
	 * @param revisedEnd   Revised end date
	 * @param order        <code>SWT.LEFT_TO_RIHT</code> or
	...
	/**
	 * Returns the status color used as part of the gradient colors used to draw the
	 * event.
	 * 
	...
	/**
	 * Sets the status color used as part of the gradient colors used to draw the
	 * event.
	 * 
	...
	/**
	 * Returns the bounds of this event (do note that events may have no bounds if
	 * they are invisible or hidden and will not get actual bounds set until they
	 * are shown).
	 * 
	 * @return Bounds
	 */
	...
	/**
	 * Sets this event to be a scope. Don't forget to add events that the scope is
	 * supposed to encompass with {@link #addScopeEvent(GanttEvent)}.
	 * 
	 * @param scope
	...
	public void addScopeEvent(final GanttEvent event) {
		...
		if (_scopeEvents.contains(event)) {
			return;
		}
	...
	public GanttEvent getEarliestScopeEvent() {
		if (!isScope() || _scopeEvents.isEmpty()) {
			return null;
		}
	...
	/**
	 * Returns what GanttGroup this event belongs to. Grouped events are drawn on
	 * the same line. Set null for no group.
	 * 
	 * @return GanttGroup
	 */
	...
	/**
	 * Sets what group this event belongs to. Grouped events are drawn on the same
	 * line.
	 * 
	...
	/**
	 * Sets if this event should be hidden or not. Hidden events are not shown on
	 * the chart and the space they normally would occupy is free.
	 * 
	...
	/**
	 * Returns whether this event is hidden or not. Hidden events are not shown on
	 * the chart and the space they normally would occupy is free.
	 * 
	...
	/**
	 * Returns whether all children are hidden or not. Children only exist on Scoped
	 * events.
	 * 
	...
	public boolean isChildrenHidden() {
		...
		for (int i = 0; i < _scopeEvents.size(); i++) {
			if (!((GanttEvent) _scopeEvents.get(i)).isHidden()) {
				return false;
			}
		}
	...
	/**
	 * Returns the full right most x value if event has text or images drawn after
	 * it.
	 * 
	 * @return x position
	 */
	...
	/**
	 * Resets the row height to be calculated automatically should the row height
	 ...
	 * 
	 * @return Row alignment, one of <code>SWT.TOP</code>, <code>SWT.CENTER</code>,
	 *         <code>SWT.BOTTOM</code>. Default is <code>SWT.TOP</code>.
	 */
	...
	/**
	 * Sets the vertical alignment for this event. Alignments are only valid if the
	 * row height is a fixed value.
	 * 
	 * @param verticalEventAlignment one of <code>SWT.TOP</code>,
	...
	/**
	 * Whether this event is resizable. If resizing is turned off in the settings
	...
	/**
	 * Whether this event is moveable. If moving is turned off in the settings this
	 * method does nothing.
	 * 
	...
	/**
	 * Returns the date prior to which no moves or resizes are allowed. Any events
	...
	/**
	 * Sets the date after which no moves or resizes are allowed. Any events moved
	 * towards or across the date mark will stop at this mark.
	 * 
	 ...
	 * <code>SWT.RIGHT</code>.
	 * 
	 * @param textLocation Text location, one of <code>SWT.LEFT</code>,
	 ...
	 * Default is <code>SWT.CENTER</code> which is right behind the event itself.
	 * 
	...
	/**
	 * Sets the vertical location where the event text will be displayed. Options
	 * are: <code>SWT.TOP</code>, <code>SWT.CENTER</code>, <code>SWT.BOTTOM</code>.
	 * Default is <code>SWT.CENTER</code>.
	 * 
	...
	/**
	 * This takes any text drawing widths into account and returns the bounds as
	 * such
	 * 
	 * @return Rectangle of bounds
	 */
	public Rectangle getActualBounds() {
		// for starters the bounds are the same as normal, then we add on text widths,
		// planned date widths etc
		...
		// if we're showing planned dates, check to see what is further away, the text
		// width or our planned date width
		// we save the values and compare to the bonuses calculated for text widths
		// below
		if (_parentComposite.isShowingPlannedDates()) {
			// note to self: we used to have date checks here, but I can't remember what
			// for, as these checks aren't necessarily valid. Revised vs. Planned
			// dates can come before one or another, doesn't matter, we still need offsets,
			// so I took them out as it fixes a reported bug as well (no bugzilla)
		...
		// now calculate text widths, later we'll check what takes up the most space,
		// planned dates or text width. Whoever is bigger
		...
		if (_parentComposite.isShowingPlannedDates()) {
			// System.err.println(plannedExtraLeft + " " + plannedExtraRight + " " +
			// usePlannedLeft + " " + usePlannedRight);
	...
	/**
	 * Call after a move/resize is done to get the command to undo/redo the latest
	 * move/resize
	 * 
	...
	IUndoRedoCommand getPostMoveOrResizeUndoCommand() {
		...
		case Constants.TYPE_MOVE:
			...
			return new EventMoveCommand(this, _preMoveDateEstiStart, _startDate, _preMoveDateEstiEnd, _endDate, _preMoveDateRevisedStart, _revisedStart, _preMoveDateRevisedEnd, _revisedEnd, gs,
					_ganttSection, _preMoveGanttSectionEventLocationIndex, indexNow);
	...
	/**
	 * Reparents this event from the current {@link GanttSection} to a new
	 * {@link GanttSection}
	 * 
	...
	/**
	 * Will update planned dates without question or internal checks. This is used
	 * internally, it's not recommended to be used externally.
	 * 
	 * @param start Date or null
	...
	/**
	 * Will update revised dates without question or internal checks. This is used
	 * internally, it's not recommended to be used externally.
	 * 
	 * @param start Date or null
	 * @param end   Date or null
	 ...
	 * The clone should be exactly like the original except for a possible few
	 * internal flags. The clone does not need to be re-added to the chart, but if
	 * it needs to exist below a certain parent it will need to be re-added to that
	 * parent (such as a scope).
	 */
	public Object clone() throws CloneNotSupportedException { // NOPMD
		...
		// it's 100% sure a clone is not being moved, we'd rather not have some funky
		// result of matching this variable
		clone._moving = false;
	...
	public void setSpecialDrawMode(SpecialDrawModes specialDrawMode) {
		this.specialDrawMode = specialDrawMode;
	}
