 ...
 * @author proshun
 */
public class ShippingCostEntityValueCalculator implements IEntityValueCalculator {
	...
	protected void evaluateCargoPNL(@NonNull EvaluationMode evaluationMode, @NonNull final ICargoValueAnnotation cargoPNLData, @NonNull final IEntity baseEntity,
		...
		for (final IPortSlot slot : cargoPNLData.getSlots()) {
			...
			addEntityBookProfit(entityPreTaxProfit, entity.getShippingBook(), hedgeValue);
			addEntityBookProfit(entityPreTaxProfit, entity.getShippingBook(), miscCostsValue);
			...
			if (slot instanceof ILoadOption) {
				...
				// // Sum up entity p&L
				// addEntityBookProfit(entityPreTaxProfit, entity.getShippingBook(), -value);
				// addEntityBookProfit(entityPreTaxProfit, entity.getShippingBook(), cargoPNLData.getSlotAdditionalShippingPNL(slot));
				// addEntityBookProfit(entityPreTaxProfit, entity.getShippingBook(), cargoPNLData.getSlotAdditionalUpsidePNL(slot));
				// addEntityBookProfit(entityPreTaxProfit, entity.getShippingBook(), cargoPNLData.getSlotAdditionalOtherPNL(slot));
				//
				...
				assert baseEntity != null;
				// addEntityBookProfit(entityPreTaxProfit, baseEntity.getShippingBook(), value);
			}
	 ...
	 */
	@Override
	public long evaluateNonCargoPlan(@NonNull EvaluationMode evaluationMode, final VoyagePlan plan, final IPortTimesRecord portTimesRecord, final IVesselAvailability vesselAvailability,
	...
	protected void calculateCargoShippingEntityCosts(@NonNull EvaluationMode evaluationMode, @NonNull final Map<IEntityBook, Long> entityPreTaxProfit,
		...
		assert false; // NBO/FBO is not costed here!
		...
		addEntityBookProfit(entityPreTaxProfit, tradingEntity.getShippingBook(), -totalShippingCosts);
	}
	...
	public long evaluateUnusedSlot(@NonNull EvaluationMode evaluationMode, @NonNull final IPortSlot portSlot, @Nullable final VolumeAllocatedSequences volumeAllocatedSequences,
		...
		long result = 0;
		{
			...
			final long postTaxValue = entity.getShippingBook().getTaxedProfit(preTaxValue, utcEquivTaxTime);
			result = postTaxValue;
