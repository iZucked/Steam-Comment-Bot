...
public abstract class EnumeratingSequenceScheduler extends AbstractLoggingSequenceScheduler {
	 ...
	 */
	protected AvailableRouteChoices[][] throughPanama;

	/**
	 * Contains the route option booking for the {@link IPortSlot}, if there is one.
	 */
	...
	protected final void prepare() {
		...
		throughPanama = new AvailableRouteChoices[size][];
		routeOptionBookings = new IRouteOptionBooking[size][];
		sizes = new int[size];

		panamaBookingsProvider.getBookings();
	 ...
	 * @return
	 */
	protected boolean[] findSequenceBreaks(final ISequence sequence, final boolean isRoundTripSequence) {
	...
	protected final void trim(final int sequenceIndex) {
		...
		Arrays.fill(throughPanama[sequenceIndex], AvailableRouteChoices.OPTIMAL);
		...
		for (final ISequenceElement element : sequence) {
			...
			if (prevElement != null) {
				...
				@NonNull
				final Pair<@NonNull ERouteOption, @NonNull Integer> minTravelTimeFromLast = distanceProvider.getQuickestTravelTime(vesselAvailability.getVessel(), prevPort, port,
						windowStartTime[index - 1], vesselAvailability.getVessel().getVesselClass().getMaxSpeed());

				minTimeToNextElement[index - 1] = durationProvider.getElementDuration(prevElement, resource) + minTravelTimeFromLast.getSecond();
			}
	...
	/**
	 * Trim time windows subject to available bookings for Panama.
	 */
	protected final void trimPanama() {

		final Map<IPort, Set<IRouteOptionBooking>> assignedBookings = new HashMap<IPort, Set<IRouteOptionBooking>>();
		final Map<IPort, Set<IRouteOptionBooking>> unassignedBookings = new HashMap<IPort, Set<IRouteOptionBooking>>();
		...
		for (int sequenceIndex = 0; sequenceIndex < sequences.size(); sequenceIndex++) {
			...
			final int[] suezTimeToNextElement = new int[size];
			final int[] panamaTimeToNextElement = new int[size];
			final boolean[] isVirtual = this.isVirtual[sequenceIndex];
			final boolean[] useTimeWindow = this.useTimeWindow[sequenceIndex];
			final boolean[] actualisedTimeWindow = this.actualisedTimeWindow[sequenceIndex];
			final boolean[] isRoundTripEnd = this.isRoundTripEnd[sequenceIndex];
			...
			for (final ISequenceElement element : sequence) {
				final IPortSlot portSlot = portSlotProvider.getPortSlot(element);
				final PortTimeWindowsRecord currentPortTimeRecord = portTimeWindowsRecord;
				// from voyageplanner --->
				final IPortSlot thisPortSlot = portSlotProvider.getPortSlot(element);
				final int visitDuration = actualsDataProvider.hasActuals(thisPortSlot) ? actualsDataProvider.getVisitDuration(thisPortSlot) : durationProvider.getElementDuration(element, resource);
				// --->
				...
				if (prevElement != null) {
					...
					directTimeToNextElement[index - 1] = durationProvider.getElementDuration(prevElement, resource) + distanceProvider.getTravelTime(ERouteOption.DIRECT,
							vesselAvailability.getVessel(), prevPort, port, windowStartTime[index - 1], vesselAvailability.getVessel().getVesselClass().getMaxSpeed());
					panamaTimeToNextElement[index - 1] = durationProvider.getElementDuration(prevElement, resource) + distanceProvider.getTravelTime(ERouteOption.PANAMA,
							vesselAvailability.getVessel(), prevPort, port, windowStartTime[index - 1], vesselAvailability.getVessel().getVesselClass().getMaxSpeed());
					suezTimeToNextElement[index - 1] = durationProvider.getElementDuration(prevElement, resource) + distanceProvider.getTravelTime(ERouteOption.SUEZ, vesselAvailability.getVessel(),
							prevPort, port, windowStartTime[index - 1], vesselAvailability.getVessel().getVesselClass().getMaxSpeed());

					minTimeToNextElement[index - 1] = Math.min(directTimeToNextElement[index - 1], Math.min(panamaTimeToNextElement[index - 1], suezTimeToNextElement[index - 1]));
				...
				if (window == null) { // empty time windows are made to be the
					...
					if (index > 0) {
						...
						// minTimeToNextElement[index - 1] = selectedTimeToNextElement[index - 1] = directTimeToNextElement[index - 1];
					...
					if (index == 0) {// first time window is special
						...
						if (useTimeWindow[index] || actualisedTimeWindow[index]) {
							...
							final int toCanal = distanceProvider.getTravelTime(ERouteOption.DIRECT, vesselAvailability.getVessel(), prevPortSlot.getPort(), routeOptionEntry,
									windowStartTime[index - 1], Math.max(panamaBookingsProvider.getSpeedToCanal(), vesselAvailability.getVessel().getVesselClass().getMaxSpeed()));
							...
							if (potentialBooking.isPresent()) {
								...
								throughPanama[sequenceIndex][index - 1] = AvailableRouteChoices.PANAMA_ONLY;
								...
								if (windowStartTime[index - 1] + toCanal + panamaBookingsProvider.getMargin() < potentialBooking.get().getBookingDate()) {
									routeOptionBookings[sequenceIndex][index - 1] = potentialBooking.get();
									currentPortTimeRecord.setRouteOptionBooking(prevPortSlot, potentialBooking.get());

									final int fromEntryPoint = distanceProvider.getTravelTime(potentialBooking.get().getRouteOption(), vesselAvailability.getVessel(),
											potentialBooking.get().getEntryPoint(), portSlot.getPort(), potentialBooking.get().getBookingDate(),
								...
								throughPanama[sequenceIndex][index - 1] = AvailableRouteChoices.OPTIMAL;
							} else if (windowStartTime[index - 1] + directTimeToNextElement[index - 1] < windowEndTime[index]
									|| windowStartTime[index - 1] + suezTimeToNextElement[index - 1] < windowEndTime[index] || directTimeToNextElement[index - 1] == minTimeToNextElement[index - 1]) {
								...
								throughPanama[sequenceIndex][index - 1] = AvailableRouteChoices.EXCLUDE_PANAMA;
								...
								for (final IRouteOptionBooking booking : set) {
									...
									if (booking.getBookingDate() + fromEntryPoint < windowEndTime[index]) {
										...
										currentPortTimeRecord.setRouteOptionBooking(prevPortSlot, booking);
								...
								if (routeOptionBookings[sequenceIndex][index - 1] == null && windowStartTime[index - 1] < panamaBookingsProvider.getStrictBoundary()) {
									minTimeToNextElement[index - 1] = Math.min(suezTimeToNextElement[index - 1], directTimeToNextElement[index - 1]);
									throughPanama[sequenceIndex][index - 1] = AvailableRouteChoices.EXCLUDE_PANAMA;
								}
				...
				if (index > 0) {
					currentPortTimeRecord.setSlotNextVoyageOptions(prevPortSlot, throughPanama[sequenceIndex][index - 1]);
				}
	...
	private final void resize(final AvailableRouteChoices[][] arrays, final int arrayIndex, final int size) {
		if ((arrays[arrayIndex] == null) || (arrays[arrayIndex].length < (size))) {
			arrays[arrayIndex] = new AvailableRouteChoices[(size)];
		}
	}
