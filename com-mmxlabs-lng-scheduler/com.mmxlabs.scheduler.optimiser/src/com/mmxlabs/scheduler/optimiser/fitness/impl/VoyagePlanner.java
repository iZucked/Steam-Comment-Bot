...
public class VoyagePlanner {
		...
		for (int idx = 0; itr.hasNext(); ++idx) {
			...
			if (breakSequence[idx]) {
				...
				if (actualsDataProvider.hasActuals(prevPortSlot)) {
					...
					if (!shortCargoEnd) {
						final int vesselCharterInRatePerDay = charterRateCalculator.getCharterRatePerDay(vesselAvailability, vesselStartTime, portTimesRecord.getFirstSlotTime());
	...
	private long generateActualsVoyagePlan(final IVesselAvailability vesselAvailability, final int vesselStartTime,
		...
		plan.setCharterInRatePerDay(charterRateCalculator.getCharterRatePerDay(vesselAvailability, vesselStartTime, portTimesRecord.getFirstSlotTime()));
