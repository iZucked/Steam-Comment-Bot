...
public class ActionSetLogger implements ILoggingDataStore {
	...
	public class ActionSetResult {
		...
		public ActionSetResult(JobState root, JobState bestResult, int noLeafs, int pnlEvaluations, int constraintEvaluations, List<Difference> changesRemaining, long runTime) {
			this.root = root;
		...
		public String jobStatePathResultRow() {
			String[] row = new String[] { "" + this.noLeafs, "" + this.pnlEvaluations, "" + this.constraintEvaluations,
					"" + StochasticActionSetUtils.getTotalPNLPerChange(this.bestResult.changeSetsAsList), "" + this.changesRemaining.size(), };
	...
	public void logRootActionSet(JobState root, JobState bestResult, int noLeafs, int pnlEvaluations, int constraintEvaluations, List<Difference> changesRemaining, long runTime) {
		rootResults.add(new ActionSetResult(root, bestResult, noLeafs, pnlEvaluations, constraintEvaluations, changesRemaining, runTime));
	}
	...
	private String percentageChangeSets(ChangeSet cs, long achievedPnLUplift) {
		...
		return String.format("\tPnL Uplift($):%s", Long.toString(dollars));
	}
	...
	private void exportAggregate(String filePath, String prefix) {
		...
		ActionSetResult acr = !rootResults.isEmpty() ? rootResults.get(rootResults.size() - 1) : null;
		List<String> text = new LinkedList<>();
		...
		if (!leafs.isEmpty()) {
			...
			text.add(String.format("%s,%s", "remainingChanges",
					Collections.min(rootResults, (o1, o2) -> Integer.compare(o1.changesRemaining.size(), o1.changesRemaining.size())).changesRemaining.size()));
		}
		PrintWriter writer = getWriter(path);
	...
	public String jobStateStringVerbose(JobState jobState) {
		...
		for (ChangeSet cs : jobState.changeSetsAsList) {
			file.add(String.format("ChangeSet: %s DeltaPnLToLast: %s DeltaPnLToBase: %s DeltaLatenessToLast: %s DeltaLatenessToBase: %s", changeSetNo++, cs.metricDelta[MetricType.PNL.ordinal()],
					cs.metricDeltaToBase[MetricType.PNL.ordinal()], cs.metricDelta[MetricType.LATENESS.ordinal()], cs.metricDeltaToBase[MetricType.LATENESS.ordinal()]));
	...
	public String jobStateStringCompact(JobState jobState) {
		List<String> file = new LinkedList<>();
		for (ChangeSet cs : jobState.changeSetsAsList) {
			String[] row = new String[] { "" + cs.changesList.size(), "" + cs.metricDelta[MetricType.PNL.ordinal()], "" + cs.metricDeltaToBase[MetricType.PNL.ordinal()],
					"" + cs.metricDelta[MetricType.LATENESS.ordinal()], "" + cs.metricDeltaToBase[MetricType.LATENESS.ordinal()] };
	...
	private @NonNull PrintWriter getWriter(final String filename) {
