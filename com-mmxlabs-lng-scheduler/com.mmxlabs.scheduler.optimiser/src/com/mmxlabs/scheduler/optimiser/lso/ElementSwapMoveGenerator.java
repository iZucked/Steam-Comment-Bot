...
public class ElementSwapMoveGenerator implements IConstrainedMoveGeneratorUnit {
	...
	public static class NullElementSwapMove extends NullMove {
	...
	public IMove generateMove() {
		...
		for (int i = 0; i < 10; ++i) {
			...
			final Pair<IResource, Integer> pos1Pair = owner.reverseLookup.get(newPair.getFirst());
			final Pair<IResource, Integer> pos2Pair = owner.reverseLookup.get(newPair.getSecond());
			...
			final ISequence seq1 = owner.sequences.getSequence(resource1);
			final ISequence seq2 = owner.sequences.getSequence(resource2);
