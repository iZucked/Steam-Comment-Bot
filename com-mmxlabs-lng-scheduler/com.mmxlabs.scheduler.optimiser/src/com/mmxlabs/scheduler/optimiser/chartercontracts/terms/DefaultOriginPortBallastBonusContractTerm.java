...
public class DefaultOriginPortBallastBonusContractTerm extends RepositioningFeeContractTerm {
	...
	public DefaultOriginPortBallastBonusContractTerm(final @NonNull IPort originPort, final @NonNull ILongCurve lumpSumCurve, final @NonNull ICurve fuelPriceCurve,
	...
	public ICharterContractTermAnnotation annotate(final IPort firstLoad, final IPortSlot lastSlot, final IVesselAvailability vesselAvailability, final int vesselStartTime, final int vesselEndTime) {
		...
		OriginPortRepositioningFeeTermAnnotation originPortBallastBonusRuleAnnotation = new OriginPortRepositioningFeeTermAnnotation();
		originPortBallastBonusRuleAnnotation.lumpSum = lumpSum;
		originPortBallastBonusRuleAnnotation.originPort = getOriginPort();
		originPortBallastBonusRuleAnnotation.distance = distanceProvider.getDistance(minTravel.getFirst(), getOriginPort() ,firstLoad, vesselAvailability.getVessel());
		originPortBallastBonusRuleAnnotation.totalTimeInHours = minTravel.getSecond();
		originPortBallastBonusRuleAnnotation.totalFuelUsed = fuelUsed;
		originPortBallastBonusRuleAnnotation.fuelPrice = fuelPriceCurve.getValueAtPoint(voyageStartTime);
		originPortBallastBonusRuleAnnotation.totalFuelCost = fuelCost;
		originPortBallastBonusRuleAnnotation.hireRate = charterRateCurve.getValueAtPoint(voyageStartTime);
		originPortBallastBonusRuleAnnotation.totalHireCost = hireCost;
		originPortBallastBonusRuleAnnotation.route = minTravel.getFirst();
		originPortBallastBonusRuleAnnotation.canalCost = canalCost;
		
		return originPortBallastBonusRuleAnnotation;
	}
