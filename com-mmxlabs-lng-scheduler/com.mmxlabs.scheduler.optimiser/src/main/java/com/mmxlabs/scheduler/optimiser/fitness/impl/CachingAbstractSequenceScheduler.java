 ...
 * clear out the cache when necessary (note to self: can we subclass weak refs
 * to get notice of when the ref is cleared and reset the associated table
 * data?)
 * 
 * @author hinton
 * 
 * @param <T>
 */
public abstract class CachingAbstractSequenceScheduler<T> extends
	...
	private final class CacheKey {
		...
		public CacheKey(IResource resource, ISequence<T> sequence,
				int[] arrivalTimes) {
			super();
			this.sequence = new ListModifiableSequence<T>(new ArrayList<T>(
					sequence.size()));
			for (T elt : sequence) {
	...
	private final LHMCache<CacheKey, Pair<Integer, List<VoyagePlan>>> cache;

	public CachingAbstractSequenceScheduler(int cacheSize) {
		super();

		cache = new LHMCache<CacheKey, Pair<Integer, List<VoyagePlan>>>(
				"Sequence Scheduler",
				new IKeyEvaluator<CacheKey, Pair<Integer, List<VoyagePlan>>>() {

					@Override
					public Pair<CacheKey, Pair<Integer, List<VoyagePlan>>> evaluate(
							CacheKey arg) {
						return new Pair<CacheKey, Pair<Integer, List<VoyagePlan>>>(
								arg, reallySchedule(arg.getResource(),
										arg.getSequence(),
										arg.getArrivalTimes()));
					}
		...
		// return reallySchedule(arg.getResource(), arg.getSequence(),
		// arg.getArrivalTimes());
		// }
		// });
	}

	private final Pair<Integer, List<VoyagePlan>> reallySchedule(
			final IResource resource, final ISequence<T> sequence,
	...
	public Pair<Integer, List<VoyagePlan>> schedule(final IResource resource,
		...
		final Pair<Integer, List<VoyagePlan>> cacheValue = cache
				.get(new CacheKey(resource, sequence, arrivalTimes));

		// final List<VoyagePlan> realValue = super.schedule(resource, sequence,
		// arrivalTimes, adjustArrivals);
