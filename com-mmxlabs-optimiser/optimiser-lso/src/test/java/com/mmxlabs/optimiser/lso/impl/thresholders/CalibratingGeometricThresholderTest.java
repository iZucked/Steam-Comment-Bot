...
public class CalibratingGeometricThresholderTest {
	...
	public void testCalibration() {
		...
		CalibratingGeometricThresholder cgt = new CalibratingGeometricThresholder(random, 20, 0.75, 0.95);
		cgt.init();
		...
		double roughTemperature = (-(base + 10)) / Math.log(0.75);
		...
		for (int i = 0; i<tests; i++) {
			long delta = base + i % 20;
			if (cgt.accept(delta)) accepts++;
		}
