...
public interface IConstraintChecker {
	...
	/**
	 * Check the {@link ISequences} object for constraint violations. Returns true
	 * if all constraints are satisfied. Returns false on a constraint violation.
	 * This version of {@link #checkConstraints} takes an input list which may be
	 * used to store informative messages about any constraint violations.
	 * 
	 * @param sequences
	 * @param messages  List which may be used to store constraint violation
	 *                  messages.
	 * @return Returns true if all constraints are satisfied.
	 */
	boolean checkConstraints(ISequences fullSequences, @Nullable Collection<IResource> changedResources, @Nullable List<String> messages);
	 ...
	 */
	default void setOptimisationData(IPhaseOptimisationData optimisationData) {
	}
}
