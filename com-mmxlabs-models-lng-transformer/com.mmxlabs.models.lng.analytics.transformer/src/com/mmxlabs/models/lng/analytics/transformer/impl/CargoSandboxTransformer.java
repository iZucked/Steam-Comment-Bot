...
public class CargoSandboxTransformer implements ICargoSandboxTransformer {
	...
	@Override
	public @Nullable
	LNGPortfolioModel createCostLine(final LNGScenarioModel scenarioModel, final ProvisionalCargo provisionalCargo) {
		...
		final IStatus validationStatus = helper.runValidation(validator, new DefaultExtraValidationContext(scenarioModel), Collections.singleton(provisionalCargo));
		...
		Schedule schedule = exporter.exportAnnotatedSolution(entities, solution);
		{
			...
			EditingDomain editingDomain = new AdapterFactoryEditingDomain(new ComposedAdapterFactory(ComposedAdapterFactory.Descriptor.Registry.INSTANCE), new BasicCommandStack());

			Command cmd = LNGSchedulerJobUtils.derive(editingDomain, scenarioModel, schedule, assignmentModel, cargoModel, null);
			cmd.execute();

			VesselAvailability availability = FleetFactory.eINSTANCE.createVesselAvailability();
			availability.setVessel(provisionalCargo.getVessel());
			availability.getStartAt().add(provisionalCargo.getBuy().getPort());
			availability.getEndAt().add(provisionalCargo.getBuy().getPort());
			availability.setStartAfter(provisionalCargo.getBuy().getDate());
			availability.setStartBy(provisionalCargo.getBuy().getDate());
			ScenarioFleetModel scenarioFleetModel = FleetFactory.eINSTANCE.createScenarioFleetModel();
			scenarioFleetModel.getVesselAvailabilities().add(availability);
			
			
			LNGPortfolioModel portfolioModel = LNGScenarioFactory.eINSTANCE.createLNGPortfolioModel();
			portfolioModel.setCargoModel(cargoModel);
			portfolioModel.setScenarioFleetModel(scenarioFleetModel);
			ScheduleModel scheduleModel = ScheduleFactory.eINSTANCE.createScheduleModel();
