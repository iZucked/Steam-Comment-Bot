...
public class ShippingCalculationsTest {
	...
	private Integer getValue(Expectations field, Event event) {
		switch(field) {
		case DURATIONS: 
	...
	public void checkPnlValues(List<? extends Event> events, CargoIndexData [] indices, Integer [] pnls) {
		if (pnls != null) {
				...
				if (pnls[i] != null) {
					for (int j = indices[i].startIndex; j <= indices[i].endIndex; j++) {
						...
						if (events.get(j) instanceof SlotVisit) {
							SlotVisit event = (SlotVisit) events.get(j);
	...
	private int getLoadDischarge(Event event) {
		if (event instanceof SlotVisit) {
			SlotVisit slotVisit = (SlotVisit) event;
			CargoAllocation ca = slotVisit.getSlotAllocation().getCargoAllocation();
	...
	public SequenceTester getDefaultTester(Class<?>[] expectedClasses) {
		
		CargoIndexData [] cargoIndices = { new CargoIndexData(3,8) };
	...
	public class SequenceTester {
		private Map<Expectations, Integer[]> expectedArrays = new HashMap<Expectations, Integer[]>();
		...
		public void setExpectedValues(Expectations field, Class<?> clazz, Integer [] values) {
		...
		public void setAllExpectedValues(Expectations field, Integer value) {
			Integer[] array = getStorageArray(field);
			for (int i = 0; i < array.length; i++) {
				array[i] = value;
			}
		...
		public void setExpectedPnlValues(Integer[] pnl) {
		...
		public void setupExpectedHireCosts(int hireRatePerHour) {
			Integer[] durations = getStorageArray(Expectations.DURATIONS);
			Integer[] hireCosts = getStorageArray(Expectations.HIRE_COSTS);
		...
		private Integer computeExpectedPnl(CargoIndexData index, double purchasePricePerM3, double salesPricePerM3, double bfPrice) {
			int result = 0;
			for (int i = index.startIndex; i <= index.endIndex; i++) {
				...
				Integer routeCost = expectedArrays.get(Expectations.ROUTE_COSTS)[i];
				result -= routeCost;
		...
		public void setupExpectedPnl(double purchasePricePerM3, double salesPricePerM3, double bfPrice) {
			Integer [] pnl = new Integer[cargoIndices.length];
	...
	public void testCanalRouteShorter() {
		...
		Integer[] expectedloadDischargeVolumes = { 10000, -9975 };
	...
	public void testCanalRouteShorterWithDelay() {
		...
		Integer[] expectedloadDischargeVolumes = { 10000, -9983 };
		checker.setExpectedValues(Expectations.LOAD_DISCHARGE, SlotVisit.class, expectedloadDischargeVolumes);
