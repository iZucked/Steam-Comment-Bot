...
public class LNGSchedulerInsertSlotJobRunner {
	...
	public SlotInsertionOptions exportSolutions(final @NonNull IMultiStateResult results, final long targetPNL, final @NonNull IProgressMonitor monitor) {
		...
		try {
			...
			AnalyticsModel analyticsModel = ScenarioModelUtil.getAnalyticsModel(originalScenarioDataProvider);
			// Lazily create analytics model if needed
			if (analyticsModel == null) {
				analyticsModel = AnalyticsFactory.eINSTANCE.createAnalyticsModel();
				cmd.append(SetCommand.create(originalEditingDomain, originalScenarioDataProvider, LNGScenarioPackage.eINSTANCE.getLNGScenarioModel_AnalyticsModel(), analyticsModel));
			}
			...
			cmd.append(SetCommand.create(originalEditingDomain, ScenarioModelUtil.getScheduleModel(originalScenarioDataProvider), SchedulePackage.Literals.SCHEDULE_MODEL__DIRTY, Boolean.FALSE));
