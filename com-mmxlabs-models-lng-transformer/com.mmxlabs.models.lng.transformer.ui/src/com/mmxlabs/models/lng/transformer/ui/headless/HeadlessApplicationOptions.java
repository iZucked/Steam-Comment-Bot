...
public class HeadlessApplicationOptions {
	...
	/**
	 * The name of the file containing config information for the algorithm being
	 * run on the scenario.
	 */
	...
	/**
	 * The number of replications to use: the algorithm will be re-run this many
	...
	/**
	 * A {@link UserSettings} object to create the algorithm's config data from, if
	...
	public enum ScenarioFileFormat {
		LINGO_FILE, CSV_FOLDER, CSV_ZIP, UNRECOGNISED, NONE
	}

	/**
	 * Returns the expected file format for the scenario, based on whether the
	 * supplied filename refers to a file or a directory, and its file extension if
	 * it is a file.
	 * 
	 * @return ScenarioFileFormat.NONE (if the file does not exist),
	 *         ScenarioFileFormat.CSV_FOLDER (for a directory),
	 *         ScenarioFileFormat.LINGO_FILE (for a file with extension ".lingo"),
	 *         ScenarioFileFormat.CSV_ZIP (for a file with extension ".zip"),
	 *         ScenarioFileFormat.UNRECOGNISED (otherwise)
	 */
	...
	/**
	 * Provides the user settings content from the userSettings field or by loading
	 * it from a "bundled" user settings file.
	 * 
	 * @return
	 */
