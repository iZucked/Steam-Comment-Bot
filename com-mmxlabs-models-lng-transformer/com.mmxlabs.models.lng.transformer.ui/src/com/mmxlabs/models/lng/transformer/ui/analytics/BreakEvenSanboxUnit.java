...
public class BreakEvenSanboxUnit {
	...
	public BreakEvenSanboxUnit(@NonNull final LNGScenarioModel scenarioModel, @NonNull final LNGDataTransformer dataTransformer, @NonNull final String phase, @NonNull final UserSettings userSettings,
		...
		modules.add(new AbstractModule() {
			...
			private BreakEvenSandboxEvaluator providePerThreadOptimiser(@NonNull final Injector injector) {
				...
				BreakEvenSandboxEvaluator optimiser = threadCache.get(Thread.currentThread());
				if (optimiser == null) {
					final PerChainUnitScopeImpl scope = injector.getInstance(PerChainUnitScopeImpl.class);
					scope.enter();
					optimiser = new BreakEvenSandboxEvaluator();
	...
	private Pair<Integer, Integer> doIt(final boolean shipped, final VesselAssignmentType vesselAssignment, final LoadSlot load, final DischargeSlot discharge, final Slot target,
		...
		} else if (vesselAssignment instanceof VesselAvailability) {
			resource = SequenceHelper.getResource(dataTransformer, (VesselAvailability) vesselAssignment);
			solution = new ModifiableSequences(CollectionsUtil.makeArrayList(resource));
			SequenceHelper.addSequence(solution, dataTransformer.getInjector(), (VesselAvailability) vesselAssignment, load, discharge);
