...
public class ExhaustiveModelChecker {
	// private Map<EClass, List<EObject>> objectsByClass = new HashMap<>();
	private Map<EStructuralFeature, Set<Object>> fieldValues = new HashMap<>();
	private Set<EObject> registeredObjects = new HashSet<>();
	private Set<EClass> allClasses = new HashSet<>();
	private Map<EClass, Set<EClass>> descendants = new HashMap<>();
	...
	/*
	 * public class OverridingSetWrapper<E> { final Set<E> additions = new HashSet<E>(); final Set<E> exclusions = new HashSet<>();
	 * 
	 * public void add(E e) { additions.add(e); exclusions.remove(e); }
	 * 
	 * public void remove(E e) { exclusions.add(e); additions.remove(e); }
	 * 
	 * public Set<E> wrap(final Set<E> set) { final HashSet<E> result = new HashSet<E>(set); result.addAll(additions); result.removeAll(exclusions); return result; } }
	 */
	...
	protected Set<Object> getRequiredValues(EStructuralFeature feature) {
		Set<Object> result = new HashSet<>(getDefaultRequiredValues(feature));
	...
	protected List<EClass> getRequiredEClasses() {
		LinkedList<EClass> result = new LinkedList<>(getDefaultRequiredEClasses());
	...
	private Map<EClass, Set<EClass>> calculateDescendants(Set<EClass> classes) {
		Map<EClass, Set<EClass>> result = new HashMap<>();
	...
	/**
	 * Returns the object to store against the containing EMF feature for a particular value of that feature. EMF objects are not stored directly since we only care about their EClass.
	 * 
	 * @param value
	 * @return
	 */
	...
	/**
	 * Records all of an object's fields, and all of its sub-objects' fields, against the EStructuralFeature objects which represent those fields. If a field is unset, the special unsetObject object
	 * is stored; if the EStructuralFeature is a reference type, the EClass of the relevant object is stored; otherwise, the actual value of the field is stored.
	 * 
	 * @param object
	 */
	private void registerObject(EObject object) {
		...
		/*
		 * // add the object to the list of objects for this class if (objectsByClass.containsKey(clazz) == false) { objectsByClass.put(clazz, new LinkedList<EObject>()); }
	...
	private Set<Object> getDefaultRequiredValues(EStructuralFeature feature) {
		...
		/*
		 * if (feature.isMany()) { result.add(emptyListObject); }
		 */
	...
	private List<String> checkFeatureValues(EStructuralFeature feature) {
		...
		if (hasActualValue == false) {
			...
			final Set<Object> missingValues = new HashSet<>(getRequiredValues(feature));
	...
	private Set<EClass> getObjectEClasses() {
		...
		final Set<EClass> objectEClasses = new HashSet<>();
	...
	/**
	 * checks a scenario to make sure that it has an exhaustive list of possible object types
	 * 
	 * this means at least one object for each instantiatable EClass every unsettable attribute must appear in both set and unset states
	 * 
	 */
	...
	public List<String> checkModel(EObject rootObject) {
		...
		Set<EStructuralFeature> checkedFeatures = new HashSet<>();

		// check each EClass
		for (EClass eClass : getRequiredEClasses()) {
			// check that the model contains an
			if (!objectEClasses.contains(eClass)) {
				problems.add(String.format("Model contains no object of class %s", eClass.getName()));
			} else {
				HashSet<EStructuralFeature> features = new HashSet<>(eClass.getEAllStructuralFeatures());
