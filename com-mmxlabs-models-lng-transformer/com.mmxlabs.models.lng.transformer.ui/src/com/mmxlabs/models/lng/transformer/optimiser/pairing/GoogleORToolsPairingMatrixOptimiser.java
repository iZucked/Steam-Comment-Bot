...
public class GoogleORToolsPairingMatrixOptimiser<P, C> implements IPairingMatrixOptimiser<P, C> {
	...
	private static long findFitness(double[][] ds, long[][] profit, boolean[] loads, boolean[] discharges) {
		long total = 0L;
		List<Double> totals = new LinkedList<>();
		for (int i = 0; i < ds.length; i++) {
			for (int j = 0; j < ds[i].length; j++) {
	...
	@Override
	public List<ConstraintInfo<P, C, ?>> findMinViolatedConstraints(PairingOptimisationData<?, ?> optimiserRecorder, long[][] values, boolean[] optionalLoads, boolean[] optionalDischarges,
			boolean[][] valid, List<ConstraintInfo<P, C, IDischargeOption>> maxDischargeSlotsConstraints, List<ConstraintInfo<P, C, IDischargeOption>> minDischargeSlotsConstraints,
