...
public class ScenarioMetaUtils {
	...
	public static int calculateShippedCargoes(List<Cargo> cargoes) {
		return (int) cargoes.stream() //
				.map(Cargo::getCargoType) //
				.filter(CargoType.FLEET::equals) //
				.count();
	}
	...
	private static Map<Integer, Integer> calculateSlotDurations(List<Slot<?>> slots) {
		...
		slots.forEach(slot -> durations.merge(slot.getSchedulingTimeWindow().getDuration(), 1, Integer::sum));

		return durations;
	}

	private static Map<String, Integer> calculateSlotWindows(List<Slot<?>> slots) {
		List<Integer> times = slots.stream().map(Slot::getSchedulingTimeWindow).map(SchedulingTimeWindow::getSizeInHours).toList();
	...
	public static ScenarioMeta writeOptimisationMetrics(IScenarioDataProvider scenarioDataProvider, UserSettings userSettings) {
