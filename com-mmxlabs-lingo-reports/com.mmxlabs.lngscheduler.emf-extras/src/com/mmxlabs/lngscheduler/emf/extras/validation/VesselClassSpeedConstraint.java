...
/**
 * Checks that the min/max speeds lie within fuel consumption curve range
...
public class VesselClassSpeedConstraint extends AbstractModelConstraint {
	...
	public IStatus validate(final IValidationContext ctx) {
		...
		if (target instanceof VesselClass) {
			...
			if (laden == null || ballast == null) return ctx.createSuccessStatus();
			
			final float maxMinSpeed = Math.max(
					getMinimumSpeed(laden),
					getMinimumSpeed(ballast));
			
			final float minMaxSpeed = Math.min(
					getMaximumSpeed(laden),
