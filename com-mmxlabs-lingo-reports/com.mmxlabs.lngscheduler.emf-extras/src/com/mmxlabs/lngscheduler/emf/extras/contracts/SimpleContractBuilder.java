 ...
 * 
 */
public class SimpleContractBuilder<T> implements IBuilderExtension {
	private final List<NetbackContract> netbacks = new LinkedList<NetbackContract>();

	ILoadPriceCalculator2 createFixedPriceContract(int pricePerMMBTU) {
		return new FixedPriceContract(pricePerMMBTU);
	}

	ILoadPriceCalculator2 createMarketPriceContract(ICurve index) {
		return new MarketPriceContract(index);
	}

	ILoadPriceCalculator2 createProfitSharingContract(ICurve actualMarket, ICurve referenceMarket, int alpha, int beta, int gamma) {
		return new ProfitSharingContract(actualMarket, referenceMarket, alpha, beta, gamma);
	}

	ILoadPriceCalculator2 createNetbackContract(int buyersMargin) {
