...
public class StartEndRequirementPortConstraintTest {
	...
	public void testValidityConstraint() {
		...
		context.checking(
				new Expectations() {{
				// The constraint will need to be able to get at the port and time's eContainer (the vessel owning it, in this case)
				atLeast(1).of(pat).eContainer();
				will(returnValue(vessel));
				// It will also need to know which feature in the vessel is containing pat (the start requirement)
				atLeast(1).of(pat).eContainingFeature();
				will(returnValue(FleetPackage.eINSTANCE.getVessel_StartRequirement()));
				// It will need to ask the vessel for its class, but this should only happen once
				atLeast(1).of(vessel).getClass_();
				will(returnValue(vesselClass));
				// And then it will need to ask the class for its inaccessible port list
				...
				// The constraint will want to ask if the port has been set (it has)
