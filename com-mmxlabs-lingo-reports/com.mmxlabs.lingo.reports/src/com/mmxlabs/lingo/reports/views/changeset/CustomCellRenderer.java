 ...
 * with custom drawing of 2 images.
 */
public class CustomCellRenderer extends GridCellRenderer
{
	int leftMargin = 4;
    ...
    private boolean issueColumn;
    
    public CustomCellRenderer(boolean issueColumn) {
    	this.issueColumn = issueColumn;
    }
    
	/*
	private int getCapacityDelta(ChangeSetTableGroup group) {
	...
	private int getNominalVesselDelta(ChangeSetTableGroup group) {
		//Check for nominal vessel changes.
		int nominalVesselCountBefore = 0;
		int nominalVesselCountAfter = 0;
		for (ChangeSetTableRow tr : group.getRows()) {
			nominalVesselCountBefore += (int)ChangeSetKPIUtil.getNominalVesselCount(tr, ResultType.Before);
			nominalVesselCountAfter += (int)ChangeSetKPIUtil.getNominalVesselCount(tr, ResultType.After);
		}
    ...
    */
	...
	private int drawSingleImage(GC gc, GridItem item, int x) {
