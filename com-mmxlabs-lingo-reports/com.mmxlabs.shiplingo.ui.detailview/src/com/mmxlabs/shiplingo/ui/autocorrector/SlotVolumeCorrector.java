...
public class SlotVolumeCorrector extends BaseCorrector {
	...
	public Pair<String, Command> correct(final Notification notification,
		...
		final ValidationSupport validationSupport = ValidationSupport
				.getInstance();
		final Scenario scenario = validationSupport
				.getScenario((EObject) target);
		if (scenario == null)
			return null;

		if (minQuantity.equals(feature)) {
			final Slot slot = (Slot) target;
			if (slot.getSlotOrContractMaxQuantity(scenario) < slot
				...
				return new Pair<String, Command>(
						...
						makeSetter(editingDomain, slot, maxQuantity,
								slot.getSlotOrContractMinQuantity(scenario)));
			}
