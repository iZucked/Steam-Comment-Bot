		...
		if (o instanceof Slot) {
			if (notification.getFeature() == CargoPackage.eINSTANCE.getSlot_Contract()) {
				...
				if (c != null) {
					...
					if (c.getDefaultPorts().isEmpty()) return null;
					if (c.getDefaultPorts().contains(p)) return null;
...
//					 TODO add a flag for default port?
					for (final Port p2 : c.getDefaultPorts()) {
						if (p2.getCapabilities().contains(capability)) {
							return new Pair<String, Command>(
									"Set port to a compatible port from " + c.getName(),
									SetCommand.create(editingDomain, s, CargoPackage.eINSTANCE.getSlot_Port(), p2)
									);
						}
					}
				}
