...
/**
 * A composite containing a form for editing Cargo instances. The EClass hierarchy is implemented by the static methods at the bottom of the class, and is not mirrored in the java class hierarchy for
...
public class CargoComposite extends AbstractDetailComposite {
	...
	protected static void createIdEditor(final AbstractDetailComposite composite, final Composite mainGroup) {
		composite.createEditorControl(mainGroup, composite.createEditor(CargoPackage.eINSTANCE.getCargo_Id()), "Id");
	}
	...
	protected static void createAllowedVesselsEditor(final AbstractDetailComposite composite, final Composite mainGroup) {
		composite.createEditorControl(mainGroup, composite.createEditor(CargoPackage.eINSTANCE.getCargo_AllowedVessels()), "Restrict To");
	}
	...
	protected static void createLoadSlotEditor(final AbstractDetailComposite composite, final Composite mainGroup) {
		final LoadSlotComposite sub = new LoadSlotComposite(composite, composite.getStyle(), "Load", false);
	...
	protected static void createDischargeSlotEditor(final AbstractDetailComposite composite, final Composite mainGroup) {
		final SlotComposite sub = new SlotComposite(composite, composite.getStyle(), "Discharge", false);
		sub.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true));
	...
	protected static void createCargoTypeEditor(final AbstractDetailComposite composite, final Composite mainGroup) {
		composite.createEditorControl(mainGroup, composite.createEditor(CargoPackage.eINSTANCE.getCargo_CargoType()), "Cargo Type");
	}
	...
	protected static void createAllowRewiringEditor(final AbstractDetailComposite composite, final Composite mainGroup) {
		composite.createEditorControl(mainGroup, composite.createEditor(CargoPackage.eINSTANCE.getCargo_AllowRewiring()), "Allow Rewiring");
	}
}
