...
public class CSVImporter {
	...
	public static MMXRootObject importCSVScenario(String baseFileName) {
		...
		dataMap.put(FleetModelImporter.FLEET_VESSELS_KEY, baseFileName + "/" + "Vessels.csv");
		...
		dataMap.put(PricingModelImporter.ROUTE_COSTS_KEY, baseFileName + "/" + "Route Costs.csv");
		...
		for (final EClass subModelClass : ManifestJointModel.getSubmodelClasses()) {
			...
			for (String key : parts.keySet()) {
				try {
					final CSVReader r = new CSVReader(dataMap.get(key));
					readers.put(key, r);
