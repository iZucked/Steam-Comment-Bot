 ...
 * <ol>
 * <li>A handler which pops up either an {@link EObjectDetailDialog} or an {@link EObjectMultiDialog} when you hit return</li>
 * <li>
 * An add action which displays an {@link EObjectDetailDialog} when you add a new element</li>
 * 
 * @author Tom Hinton
 * 
 */
public class ScenarioObjectEditorViewerPane extends EObjectEditorViewerPane {
	final static protected EAttribute namedObjectName = ScenarioPackage.eINSTANCE.getNamedObject_Name();

	/**
	 * @param page
	 * @param part
	 */
	public ScenarioObjectEditorViewerPane(final IWorkbenchPage page, final ScenarioEditor part) {
		super(page, part);
	}

	/**
	 * A convenience version of {@link #init(List, AdapterFactory)} which takes varargs.
	 * 
	...
	/**
	 * Create a custom add action, which delegates to the default behaviour to create the object, but adds in an editor dialog.
	 */
	@Override
	protected Action createAddAction(final GridTableViewer viewer, final EditingDomain editingDomain, final EMFPath contentPath) {
		final AddAction delegate = (AddAction) super.createAddAction(viewer, editingDomain, contentPath);

		final Action result = new AddAction(editingDomain, contentPath.getTargetType().getName()) {
			@Override
			public EObject createObject(final boolean usingSelection) {
				final EObject newObject = delegate.createObject(usingSelection);

				try {
					ValidationSupport.getInstance().setContainers(Collections.singletonList(newObject), (EObject) getOwner(), (EReference) getFeature());

					final DetailCompositeDialog dcd = new DetailCompositeDialog(viewer.getControl().getShell(), part, part.getEditingDomain());
					...
					ValidationSupport.getInstance().clearContainers(Collections.singletonList(newObject));
				}
			}
				...
				if (e.keyCode == '\r') {
					if (selection instanceof IStructuredSelection) {
						...
						if (!isLockedForEditing() && ((l.size() > 1) && ((e.stateMask & SWT.CONTROL) == 0))) {
							final MultiDetailDialog multiDialog = new MultiDetailDialog(v.getControl().getShell(), part, part.getEditingDomain());
							...
							final DetailCompositeDialog dcd = new DetailCompositeDialog(v.getControl().getShell(), part, part.getEditingDomain());
