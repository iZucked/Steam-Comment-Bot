		...
		if (window.getActivePage().getActiveEditor() instanceof ScenarioEditor) {
			...
			if (curveDialog.open(current) == CurveDialog.OK) {
				if (current == null) {
					...
					cc.append(AddCommand.create(editor.getEditingDomain(),
							scenario.getOptimisation(),
							OptimiserPackage.eINSTANCE
									.getOptimisation_DiscountCurves(), newCurve));
					
					cc.append(SetCommand.create(editor.getEditingDomain(), scenario.getOptimisation().getCurrentSettings(),
							OptimiserPackage.eINSTANCE.getOptimisationSettings_DefaultDiscountCurve(),
							newCurve));
