 ...
 *******************************************************************************/
package com.mmxlabs.lngscheduler.emf.editor.importer;
...
public class CSVImportWizard extends Wizard implements IImportWizard {
	...
	public boolean performFinish() {
		...
		if (inputFilePaths.containsKey(FleetPackage.eINSTANCE.getVesselClass()) &&
				inputFilePaths.containsKey(FleetPackage.eINSTANCE.getFuelConsumptionLine())) {
			// process these first because of special file format
			final ImportSession vcSession = new ImportSession();
			vcSession.setDeferredReferences(refs);
			vcSession.setNamedObjectRegistry(nor);
			vcSession.setInputFileName(inputFilePaths.get(FleetPackage.eINSTANCE.getVesselClass()));
			vcSession.setOutputObjectClass(FleetPackage.eINSTANCE.getVesselClass());
			vcSession.run();
			for (final EObject vc : vcSession.getImportedObjects()) {
				nor.addEObject(vc);
			}
			final ImportSession fcSession = new ImportSession();
			fcSession.setDeferredReferences(refs);
			fcSession.setNamedObjectRegistry(nor);
			fcSession.setInputFileName(inputFilePaths.get(FleetPackage.eINSTANCE.getFuelConsumptionLine()));
			fcSession.setOutputObjectClass(FleetPackage.eINSTANCE.getFuelConsumptionLine());
			fcSession.run();
			importedObjects.addAll(vcSession.getImportedObjects());
		}
		
		// process the rest
		for (final Map.Entry<EClass, String> job : inputFilePaths.entrySet()) {
			final ImportSession session = new ImportSession();
			session.setDeferredReferences(refs);
			session.setNamedObjectRegistry(nor);
			session.setInputFileName(job.getValue());
			session.setOutputObjectClass(job.getKey());
			session.run();
			importedObjects.addAll(session.getImportedObjects());
		}
		...
		scenario.setContractModel(ContractFactory.eINSTANCE.createContractModel());
		...
		// and last of all put all the results into a new scenario according to their kind.
		for (final EObject object : importedObjects) {
			if (object != null) {
		...
		inputPage = new WizardPage("Choose CSV Data to Import") {
			...
			public void createControl(final Composite parent) {
				...
				final String [] extensions = new String[] {"*.csv"};
				{
					...
					final FileFieldEditor portsEditor = new FileFieldEditor("portsSelect", "Ports", group);
					portsEditor.setFileExtensions(extensions);
					portsEditor.setEmptyStringAllowed(true);
					portsEditor.getTextControl(group).addModifyListener(listener(
							PortPackage.eINSTANCE.getPort()
					));
					final FileFieldEditor dmEditor = new FileFieldEditor("dmSelect", "Distance Matrix", group);
					dmEditor.setFileExtensions(extensions);
					dmEditor.setEmptyStringAllowed(true);
					dmEditor.getTextControl(group).addModifyListener(listener(
				...
				
				{
					...
					group.setText("Fleet");
					group.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false));
					final FileFieldEditor vcEditor = new FileFieldEditor("vcSelect", "Vessel Classes", group);
					...
					final FileFieldEditor fcEditor = new FileFieldEditor("fcSelect", "Fuel Curves", group);
					fcEditor.setFileExtensions(extensions);
					fcEditor.setEmptyStringAllowed(true);
					fcEditor.getTextControl(group).addModifyListener(listener(
							FleetPackage.eINSTANCE.getFuelConsumptionLine()
					));
					final FileFieldEditor vEditor = new FileFieldEditor("vSelect", "Vessels", group);
				...
				
				{
					...
					group.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false));
					final FileFieldEditor marketEditor = new FileFieldEditor("mSelect", "Markets", group);
					...
					final FileFieldEditor contractEditor = new FileFieldEditor("cSelect", "Contracts", group);
					contractEditor.setFileExtensions(extensions);
					contractEditor.setEmptyStringAllowed(true);
					contractEditor.getTextControl(group).addModifyListener(listener(
							ContractPackage.eINSTANCE.getContract()
					));
					final FileFieldEditor entityEditor = new FileFieldEditor("eSelect", "Entities", group);
				...
				
				{
					...
					group.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false));
					final FileFieldEditor cargoEditor = new FileFieldEditor("cgSelect", "Cargoes", group);
					...
					final FileFieldEditor eventEditor = new FileFieldEditor("veSelect", "Events", group);
