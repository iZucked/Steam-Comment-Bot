...
print('Searching Pricing Calendar CSV in the project...')
...
print('File was read successfully! Adding new data...')

for calendar in calendars:

    exchange_name = calendar['name']

    rows.append([exchange_name, calendar['title'], '', '', ''])
    ...
    for year in years_to_append:
        for month in range(1, 13):
            ...
            start_day, start_month= tuple(map(eval, calendar['start'].split()))
            end_day, end_month = tuple(map(eval, calendar['end'].split()))
            ...
            start_date = f'{"%02d" % (start_day,)}/{"%02d" % (start_month,)}/{year}'
            end_date = f'{"%02d" % (end_day,)}/{"%02d" % (end_month,)}/{year}'
...
print('Data sorted! Writing it back...')
